...
`configs: [configReadModel]`). `AllInOne` reste `FullOwner`; `SplitInfraCode` sépare le bootstrap
`FullOwner` infra du bootstrap `ApplicationOnly` applicatif. Le bootstrap projet est gardé
...
| Lot 3 | Push bootstrap niveau config | Done | dotnet-dev | 2026-08-28 | Nouveau slice `InfrastructureConfig/Commands/PushBootstrapToGit/` (Command/Validator/Handler) mirroring exact `PushPipelineToGit`, résout via `targetResolver.Resolve(project, config, ArtifactKind.Bootstrap)`, `BasePath = target.PipelineBasePath`, fichiers poussés sans `GeneratedPipelinePathNormalizer.Normalize` (bootstrap.pipeline.yml est un fichier plat unique à la racine du pipeline base path — normalizer non applicable, différence documentée dans le XML doc du handler). Le handler exige maintenant le secret du dépôt résolu et, en `SplitInfraCode`, ne pousse que le bucket `infra/`. 12 tests ajoutés (6 handler + 6 validator) — tous GREEN. `dotnet build` 0 erreur. |
...
| Lot 7 | Validation build/tests/exécution réelle | In progress | dev (orchestrateur) | 2026-09-01 | Validation automatisée D09 + D10 passée : solution build 0 erreur, tests Application ciblés verts pour les deux orchestrateurs, test API de rate limiting vert, tests Infrastructure ciblés verts pour Key Vault/Blob/providers, `npm run typecheck` + `npm run build` 0 erreur, 70 specs Angular ciblées vertes. Les contrats d’annulation propagent désormais l’arrêt et aucun flux bulk ne tente la cible suivante. La suite .NET complète reste limitée par Docker/PostgreSQL (4 346 tests, 56 échecs) et la suite Karma s’interrompt après 351/453 specs avec 15 échecs hors périmètre. **Reste à faire** : une seule campagne réelle contre un projet MultiRepo et un fournisseur Git pour valider PAT config-level, génération Bicep/pipeline/bootstrap, push AllInOne, push SplitInfraCode et échec partiel. |
...
- [x] Les lots 1 à 6 étaient à `Done` avant le démarrage de D10; le push bulk utilise désormais
  les artefacts bootstrap config-level du lot 3.
