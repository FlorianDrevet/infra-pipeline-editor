...
2. **Run the validator** (optional but recommended):
   ```bash
   python .Codex/skills/draw-io-diagram-generator/scripts/validate-drawio.py <path-to-file.drawio>
   ```
...
```bash
python .Codex/skills/draw-io-diagram-generator/scripts/add-shape.py docs/arch.drawio "New Service" 700 380
```
...
```bash
python .Codex/skills/draw-io-diagram-generator/scripts/validate-drawio.py <file.drawio>
```
...
All companion files are in `.Codex/skills/draw-io-diagram-generator/`:
