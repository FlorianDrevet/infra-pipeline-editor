...
description: "Use when: code exploration via knowledge graph, execution-flow discovery, impact/blast-radius analysis before modifying a shared symbol, post-change validation, or safe refactoring with GitNexus MCP."
---
...
| "Quels flux d'exécution existent dans le projet ?" | `gitnexus://repo/infra-pipeline-editor/processes` | Lecture manuelle de .claude/memory/MEMORY.md (incomplet) |
...
- Les résultats d'exploration GitNexus (clusters importants, flows critiques, symboles à haut risque) sont persistés dans `.claude/memory/13-code-graph.md` par `@dream`
