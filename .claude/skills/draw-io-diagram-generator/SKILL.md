...
2. **Run the validator** (optional but recommended):
   ```bash
   python .claude/skills/draw-io-diagram-generator/scripts/validate-drawio.py <path-to-file.drawio>
   ```
...
```bash
python .claude/skills/draw-io-diagram-generator/scripts/add-shape.py docs/arch.drawio "New Service" 700 380
```
...
```bash
python .claude/skills/draw-io-diagram-generator/scripts/validate-drawio.py <file.drawio>
```
...
All companion files are in `.claude/skills/draw-io-diagram-generator/`:
