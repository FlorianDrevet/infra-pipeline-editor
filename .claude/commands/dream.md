---
description: Consolidation manuelle de la mémoire projet — synthétise, déduplique et prune .claude/memory/. À lancer quand la mémoire a grossi ou divergé.
---
...
Le sous-agent `dream` exécute ses 4 phases (Orient → Gather → Consolidate → Prune) sur `.claude/memory/` uniquement :
1. **Orient** — lit `MEMORY.md` + survole les fichiers thématiques.
2. **Gather** — récupère le signal récent (`changelog.md`, `git log --since`, `gitnexus_detect_changes`).
...
À la fin, le sous-agent met à jour `lastDreamDate` dans `.claude/memory/dream-state.md` et renvoie un Dream Report.
