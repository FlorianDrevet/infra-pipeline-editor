...
- App pipeline Phase 5 Sonar/Linting switches [2026-05-21]: shared app templates now include `app-sonar-analysis.step.yml` (SonarQubePrepare@6 + SonarQubeAnalyze@6 + SonarQubePublish@6) and `app-linting.step.yml` (PowerShell Invoke-Expression). CI code wrappers/pipelines/jobs forward `runSonarAnalysis`, `sonarProjectKey`, `sonarOrganization`, `sonarServiceConnection`, `runLinting`, and `lintCommand`.
- App pipeline Phase 6 License/Notification switches [2026-05-21]: shared app templates now include `app-license-check.step.yml` (multi-tool: dotnet-delice, license-checker, licensefinder) and `app-notification.step.yml` (webhook POST with pipeline status). CI code wrappers/pipelines/jobs forward `runLicenseCheck`/`licenseCheckTool`; both CI and release wrappers/pipelines/jobs forward `enableNotifications`/`notificationWebhookUrl`. Total shared path count = 31.
