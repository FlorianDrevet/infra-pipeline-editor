...
- **Triage 2026-08-28**: `PrivateDnsZone` stays demolished — its job (DNS resolution for a Private Endpoint) is covered by V3's `PrivateEndpointDnsMode` (`AutoManaged`/`ExistingHub`); residual error constant + migration mentions are to be purged during repair. `NetworkSecurityGroup` and `FrontDoor` are **not** dead-and-forgotten: nothing today covers NSG rule editing or Front Door (CDN/WAF/routing), and the project owner confirmed intent to support both again as first-class resources on the roadmap — `Subnet.NsgId` (the only NSG residue) must stay untouched until that work is picked up. See `docs/stabilization/feature-map.md`, "Décisions de tri".
...
- **PAT storage for config-level repositories [2026-08-28, D01 fix]**: `InfraConfigRepository` (config-level, MultiRepo) now stores its PAT in Key Vault exactly like `ProjectRepository` does, mirrored one-for-one: `ProjectGitSecretNames.GetInfraConfigRepositoryPatSecretName(InfraConfigRepositoryId)` (same `git-pat-repo-` prefix, no collision risk — both id types wrap globally unique GUIDs). `AddInfraConfigRepositoryCommand`/`UpdateInfraConfigRepositoryCommand` gained `PersonalAccessToken`; their handlers now require+verify it via `ProjectRepositoryConnectionVerifier` (reused as-is from `Application.Projects.Common`, same assembly) whenever `HasCompleteConnectionDetails` is true — which for config repos is unconditionally true, since `InfrastructureConfig.AddRepository`/`UpdateRepository` (unlike `Project.AddRepository`) take non-nullable `providerType`/`repositoryUrl`/`defaultBranch` and have no slot/partial-declaration mode. `RepositoryTargetResolver.BuildFromConfigRepository` now returns the real secret name instead of `PatSecretName: null`, which was the actual root cause of connection-test/branch-listing/push failures in MultiRepo (D01). Contracts `Add/UpdateInfraConfigRepositoryRequest` and `ProjectController` wired through. D09 (bootstrap unreachable) and D10 (push rejects MultiRepo) are separate, not covered by this fix.
