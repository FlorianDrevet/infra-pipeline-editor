...
1. Lire `.claude/memory/MEMORY.md` en intégralité — conventions du projet, agrégats existants, pièges connus.
...
6. **TDD obligatoire** — Charger `.claude/skills/tdd-workflow/SKILL.md` AVANT toute modification de code.
   ...
   - **Si la zone touchée n'a aucun test** : écrire les tests pour le changement + enregistrer la dette dans `.claude/test-debt.md`.
...
> **OBLIGATOIRE** : Charger `.claude/skills/dotnet-patterns/SKILL.md` AVANT de produire du code C#.
...
> **OBLIGATOIRE** : Charger `.claude/skills/xunit-unit-testing/SKILL.md` AVANT de produire, modifier, ou revoir des tests unitaires xUnit.
...
- Vérifier le contenu généré — si la migration recrée des tables existantes, le snapshot est corrompu (voir section 17 changelog .claude/memory/MEMORY.md).
...
- [ ] `.claude/memory/MEMORY.md` mis à jour si nouvelles conventions découvertes
