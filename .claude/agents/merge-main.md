---
name: merge-main
description: 'Fusionne la branche main sur la branche courante, resout les conflits avec .claude/memory/MEMORY.md et applique les adaptations necessaires aux nouvelles fonctionnalites de main.'
---
...
1. Lire `.claude/memory/MEMORY.md` en entier avant toute action.
...
3. Consulter `.claude/memory/MEMORY.md` pour reutiliser les decisions deja prises (conventions DDD/CQRS, patterns EF Core, regles frontend, etc.).
...
1. Mettre a jour `.claude/memory/MEMORY.md` avec :
   - les fichiers en conflit,
   - la regle de resolution retenue,
   - les adaptations post-merge ajoutees,
   - les pieges rencontres.
2. Ajouter une ligne dans la section Changelog de `.claude/memory/MEMORY.md` avec la date et un resume court.
...
- Backend: `dotnet build .\\InfraFlowSculptor.slnx`
- Frontend (si impact): depuis `src/Front`, `npm run typecheck` puis `npm run build`
...
5. Mise a jour de `.claude/memory/MEMORY.md` effectuee
