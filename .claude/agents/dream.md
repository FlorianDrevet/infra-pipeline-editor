---
name: dream
description: 'Consolidation mémoire (Dream). Synthétise les informations récentes en mémoire durable. À lancer par le thread principal (commande /dev) quand les gates sont satisfaites.'
---
...
Ce sous-agent est lancé **manuellement** par le thread principal via la commande **`/dream`**, quand l'utilisateur juge que la mémoire a besoin d'être consolidée (après une grosse session, plusieurs features, ou si l'index a gonflé).

Plus de gate automatique, plus de compteur de sessions, plus de verrou filesystem : la consolidation est un acte volontaire. `dream-state.md` ne sert plus qu'à mémoriser `lastDreamDate` (date de la dernière consolidation, à titre informatif).

> **Note Claude Code :** un sous-agent ne peut pas en lancer un autre. C'est donc le thread principal qui lance `dream` via le tool Agent, puis reprend la main une fois la consolidation terminée.
...
1. Lire `.claude/memory/dream-state.md` (pour info : `lastDreamDate`)
2. Lister le contenu de `.claude/memory/` (tous les fichiers thématiques)
3. Lire `.claude/memory/MEMORY.md` (l'index léger)
4. Survoler chaque fichier thématique (`01-*.md` à `14-*.md` + `changelog.md`) pour identifier les zones à améliorer
...
6. **Mettre à jour `.claude/memory/13-code-graph.md`** — Si GitNexus a révélé de nouveaux clusters importants, flows critiques, ou symboles à haut risque (beaucoup de dépendants upstream), les ajouter dans ce fichier. Supprimer les entrées qui ne correspondent plus au graphe.
...
2. **`.claude/memory/MEMORY.md` (index)** doit rester < 80 lignes. Il ne contient QUE :
...
4. **Cohérence de l'index** — Vérifier que chaque fichier dans `.claude/memory/` est référencé dans `.claude/memory/MEMORY.md`

---

## Règles strictes

- **NE PAS** modifier de fichiers en dehors de `.claude/memory/` et `.claude/memory/MEMORY.md`
- **NE PAS** modifier de code source du projet
- **NE PAS** supprimer un fichier thématique entier (condenser plutôt)
- **Toujours** mettre à jour `dream-state.md` en fin de dream : `lastDreamDate` = date du jour
...
- Index .claude/memory/MEMORY.md : N lignes
```
