...
- Lire `.claude/memory/MEMORY.md` en entier avant tout diagnostic.
...
- Démarrer AppHost si nécessaire (`aspire run` ou `dotnet run --project .\src\Aspire\InfraFlowSculptor.AppHost\InfraFlowSculptor.AppHost.csproj`).
...
- Mettre à jour `.claude/memory/MEMORY.md` (convention/piège/incident + ligne de changelog).
...
- [ ] `.claude/memory/MEMORY.md` mis à jour + changelog ajouté
