...
| `docs`     | Documentation uniquement (README, .claude/memory/MEMORY.md, commentaires) |
...
docs: update .claude/memory/MEMORY.md with StorageAccount conventions
```
...
6. **Vérifier et mettre à jour la documentation** si les changements concernent la documentation ou s'il y a des informations à ajouter :
   ...
   - Vérifier les sections pertinentes dans `docs/`, `docs/azure/`, `README.md` ou `.claude/memory/MEMORY.md`
...
7. **Mettre à jour `.claude/memory/MEMORY.md`** et inclure cette mise à jour dans la même PR.
...
outil : mcp_microsoft_azu_wit_add_child_work_items
  ...
  workItemType : "Task"
  items:
    - title      : "<Action réalisée — ex: Créer merge-main.md>"
...
| `chore(agents)` | Créer fichier agent, Mettre à jour .claude/memory/MEMORY.md, Supprimer fichiers obsolètes |
