...
1. **Lire `.claude/memory/MEMORY.md`** — pour connaître les conventions et l'état du projet.
2. **Lire ce fichier en entier** — pour appliquer les règles Angular du projet.
3. **Charger le skill UI/UX** `ui-ux-front-saas` (`.claude/skills/ui-ux-front-saas/SKILL.md`) pour toute tâche qui touche un écran, composant, layout, HTML ou SCSS.
...
8. **Analyse d'impact Codegraph** — Avant de modifier un service partagé (`shared/services/`) consommé par plusieurs composants, exécuter `codegraph_impact("ServiceName")` pour identifier tous les composants consommateurs. Si risque élevé → alerter l'utilisateur.
...
- Le skill `.claude/skills/ui-ux-front-saas/SKILL.md` est la référence UX/UI du projet.
...
> **OBLIGATOIRE** : Charger `.claude/skills/angular-patterns/SKILL.md` AVANT de produire du code Angular.
...
- [ ] Lu `.claude/memory/MEMORY.md` avant de commencer
...
2. Vérifier `git diff` — confirmer que seuls les fichiers/flux attendus sont modifiés. Si doute, utiliser `codegraph_impact("Symbol")` sur les symboles modifiés.
...
5. Si la zone touchée n'a aucun test et qu'un service critique ou une logique métier est modifié, enregistrer la dette dans `.claude/test-debt.md` avec le préfixe `Front/`.
6. Documenter les nouveaux composants/services/interfaces dans `.claude/memory/MEMORY.md` section 13.
