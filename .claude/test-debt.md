...
- [2026-08-28] **Phase 0 (build/tests/stack) validée pour la 1ère fois depuis le début de la stabilisation.** `dotnet build` : 0 erreur (168 warnings pré-existants). `dotnet test` sur toute la solution : 3827 tests, **51 échecs**. Répartition :
  - **44 dans `Application.Tests`** — confirmé être la dette déjà trackée le 2026-06-03 (ligne ci-dessous, "38 autres failures préexistantes"), pas une régression neuve. Root cause vérifiée sur `GetKeyVaultQueryHandlerTests` (représentatif du lot) : le test mocke encore un `IResourceGroupRepository` séparé et n'hydrate jamais `_keyVault.ResourceGroup` — mais le handler actuel lit `keyVault.ResourceGroup!.InfraConfigId` directement sur l'agrégat déjà chargé (suite au fix repo `.Include(ResourceGroup)` du 2026-06-02). Le test est resté sur l'ancienne forme du handler. Dette pure, pas un bug de prod — même schéma probable sur tout le lot des ~44 (`Get<X>QueryHandlerTests` sur la quasi-totalité des types de ressource).
  - **4 dans `Api.Tests`** (`SecurityMiddlewareIntegrationTests`) + **1 dans `Contracts.Tests`** (`UpdateProjectEnvironmentRequestTests`) — déjà connus/documentés dans plusieurs entrées de `changelog.md` comme "5 known out-of-scope failures".
  - **1 nouveau dans `Contracts.Tests`** (`ContractsResponseShapeSnapshotTests`) : snapshot figée attend 102 types de réponse publics, le code n'en a plus que 101 — drift à reconcilier (identifier quel DTO a disparu, ré-approuver le snapshot).
  - **1 nouveau dans `Infrastructure.Tests`** (`CoreStringLengthConfigurationTests`) : `Subnet.AddressPrefix` n'a pas de `HasMaxLength` explicite en config EF — vrai petit gap, garde-fou qui fait son travail.
  - Frontend : `node_modules/typescript` était tronqué (pas de dossier `bin/`, cassait `npm run typecheck`) — corrigé par réinstallation ciblée du package. `npm run typecheck` maintenant propre.
  - Stack Aspire : démarrage propre confirmé (Postgres, Azurite, émulateur Key Vault, DbGate tous `Up`, dashboard répond, 0 erreur/exception dans les logs de démarrage).
- [2026-06-03] **P1 — Application.Tests / StorageAccount AddBlobContainer+Queue+Table handlers**: Tests préexistants cassés (6/9) car `StorageAccountAccessHelper` vérifie `storageAccount.ResourceGroup is null` et les tests ne peuplaient pas cette navigation (EF uniquement). Corrigés dans cette tâche (BUG 3) via réflexion pour hydrater la propriété. 38 autres failures préexistantes hors périmètre dans `InfraFlowSculptor.Application.Tests` (CosmosDb, SqlDatabase, EventHubNamespace, VirtualNetwork validators, etc.) — à résorber séparément.
- [2026-06-03] **P1 — Infrastructure.Tests / StorageAccountRepository**: Les méthodes `AddBlobContainer`, `AddQueue`, `AddTable` (ex-Async) ne sont pas couvertes par des tests unitaires d'infrastructure. Le `StorageAccountRepository` manque d'un projet `InfraFlowSculptor.Infrastructure.Tests` dédié pour ces méthodes synchrones.
...
| 47 | `InfraFlowSculptor.Application` | `AddSubnetCommandHandler`, `UpdateSubnetCommandHandler`, `RemoveSubnetCommandHandler` | Handlers couverts par 19 nouveaux tests (6 + 7 + 5) : happy path, VNet not found, ResourceGroup not found, accès refusé, delegation null/valide, PrivateEndpointPolicy valide, SubnetId correct, subnet inconnu. Couverture complète des garde-fous APP-015 (Enum.Parse). | P1 | 2026-06-03 | 2026-06-03 | dotnet-dev |
| 48 | `InfraFlowSculptor.Application` | `DocumentIntelligence` — tous les handlers et la query | 19 tests écrits : CreateCommandHandler (5), UpdateCommandHandler (6), DeleteCommandHandler (4), GetQueryHandler (3). Chemins couverts : happy path, ResourceGroup not found, DocumentIntelligence not found, accès refusé (write/read), env settings invalides (SKU invalide), env settings valides, IsExisting flag. Accès navigation `ResourceGroup.InfraConfigId` simulé via réflexion (`typeof(AzureResource).GetProperty("ResourceGroup").SetValue(...)`). | P1 | 2026-06-03 | 2026-06-03 | dotnet-dev |
| 49 | `InfraFlowSculptor.Domain` | `DocumentIntelligenceAggregate` — agrégat + entity | 17 tests écrits : DocumentIntelligenceTests (12) couvrant Create, Update, SetEnvironmentSettings, SetAllEnvironmentSettings + guards IsExisting ; DocumentIntelligenceEnvironmentSettingsTests (5) couvrant Create, Update, ToDictionary (all overrides / no overrides / sku-only). | P1 | 2026-06-03 | 2026-06-03 | dotnet-dev |
