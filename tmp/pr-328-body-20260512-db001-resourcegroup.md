...
Étendre la PR #328 au-dela du lot securite/validation initial avec plusieurs remediations d'audit substantielles et validees: modernisation du rate limiting, sweep DB-001 complet en trois tranches, reclassement propre de DB-002 comme obsolete apres verification du modele relationnel, suppression du N+1 critique de APP-003 sur les references croisees de configuration, plusieurs splits read-only `DB-003`, correction du cascade-delete `DB-006` sur `AppSetting`, durcissement `APP-002` sur la generation/download pipeline puis sur la creation de projet self-service (retour `401` explicite si `CurrentUser` n'est pas resolu), quatre tranches successives `APP-005`, et suppression du warning `NU1904` via le pin `Microsoft.AspNetCore.DataProtection` 10.0.7.
...
- Mise a jour de audits/triage-2026-05-12.md : #185 reste ouvert mais documente maintenant quatre tranches a levier (`IAppPipelineRequestFactory`, `BlobDownloadHelper`, la reutilisation du `Project` deja autorise pour la garde `CanGenerateAllFromProjectLevel()`, puis un reload enrichi unique `GetByIdWithAllAndPipelineVariableGroupsAsync(...)`) avant d'attaquer les autres god handlers.
...
- [x] dotnet test .\InfraFlowSculptor.slnx (2019 total, 0 failed, 9 skipped)
- [x] Tests cibles verts pour GeneratedFilePathValidationTests, ValidationBehaviorTests, EnumValidationTests, CoreStringLengthConfigurationTests, IndexCoverageConfigurationTests, DeleteBehaviorConfigurationTests, CreateProjectCommandValidatorTests, CreateProjectRequestTests, CreateInfrastructureConfigCommandValidatorTests, CreateInfrastructureConfigRequestTests, CreateResourceGroupCommandValidatorTests, CreateResourceGroupRequestTests, ListCrossConfigReferencesQueryHandlerTests, `ProjectAccessServiceTests`, `InfraConfigAccessServiceTests`, `ProjectRepositoryTests`, `InfrastructureConfigRepositoryTests`, `GetResourceGroupQueryHandlerTests`, `ListResourceGroupResourcesQueryHandlerTests`, `ResourceGroupRepositoryTests`, les nouveaux tests DB-003 KeyVault / Redis / Storage, `GeneratePipelineCommandHandlerTests`, `DownloadPipelineCommandHandlerTests`, `AppPipelineRequestFactoryTests`, et le test repository cible de `GetConfigSummariesByIdsAsync(...)`
