...
Étendre la PR #328 au-dela du lot securite/validation initial avec plusieurs remediations d'audit substantielles et validees: modernisation du rate limiting, sweep DB-001 complet en trois tranches, reclassement propre de DB-002 comme obsolete apres verification du modele relationnel, suppression du N+1 critique de APP-003 sur les references croisees de configuration, plusieurs splits read-only `DB-003`, correction du cascade-delete `DB-006` sur `AppSetting`, durcissement `APP-002` sur la generation/download pipeline puis sur la creation de projet self-service (retour `401` explicite si `CurrentUser` n'est pas resolu), cinq tranches successives `APP-005`, un nouveau slice `APP-001` sur les validators manquants de push projet, et suppression du warning `NU1904` via le pin `Microsoft.AspNetCore.DataProtection` 10.0.7.
...
- Ajout des validators manquants `PushProjectBootstrapPipelineToGitCommandValidator` et `PushProjectGeneratedArtifactsToGitCommandValidator`, alignes sur les regles deja appliquees aux commandes soeurs `PushProjectBicepToGit` / `PushProjectPipelineToGit`.
...
- `InfraConfigAccessService.VerifyReadAccessAsync(...)` utilise desormais un lookup infrastructure config dedie no-tracking (`GetByIdReadOnlyAsync`) au lieu de reemployer `GetByIdAsync(...)`; `VerifyWriteAccessAsync(...)` reste tracke pour les flows d'ecriture qui reutilisent ensuite l'aggregate charge.
...
- `MultiScopeGitPushRequestBuilder` centralise maintenant entre `PushProjectGeneratedArtifactsToGitCommandHandler` et `PushProjectArtifactsToMultiRepoCommandHandler` la fusion des scopes Git, le re-slicing des chemins root-level et la detection des collisions de contenus sur un meme chemin resolu.
- `GenerateProjectBicepCommandHandler` et `GenerateProjectPipelineCommandHandler` reutilisent maintenant le `Project` deja renvoye par `IProjectAccessService.VerifyWriteAccessAsync(...)` pour appliquer la garde `CanGenerateAllFromProjectLevel()`, ce qui supprime un rechargement du projet et un chargement inutile des `InfrastructureConfig` domaine juste pour ce gate d'ambiguite.
- `GenerateProjectPipelineCommandHandler` et `GenerateProjectBootstrapPipelineCommandHandler` reutilisent maintenant un lookup enrichi unique `IProjectRepository.GetByIdWithAllAndPipelineVariableGroupsAsync(...)` quand ils ont besoin a la fois du routage Git projet et des variable groups, au lieu d'enchaîner `GetByIdWithAllAsync(...)` et `GetByIdWithPipelineVariableGroupsAsync(...)` sur le meme aggregate.
...
- DB-001 tranche ParameterDefinition: `ParameterDefinition.Name` est desormais contraint a varchar(100), `ParameterDefinition.Type` a varchar(20), et `ParameterDefinition.DefaultValue` a varchar(500), via `ParameterDefinitionConfiguration` + migration dediee.
- Generation des migrations EF Core AddCoreStringLengthConstraints, AddResourceGroupNameLengthConstraint puis AddParameterDefinitionLengthConstraints pour materialiser l'ensemble du sweep DB-001 en base.
...
- Ajout de `IInfrastructureConfigRepository.GetByIdReadOnlyAsync(...)` pour isoler un nouveau slice `DB-003` cote acces lecture des infrastructure configs, sans casser `GetByIdAsync(...)` encore partage par les flows write.
...
- Ajout de tests de metadonnees EF (CoreStringLengthConfigurationTests) pour verrouiller la matrice de longueurs sur le noyau DB-001, y compris `ResourceGroup.Name = 90` et `ParameterDefinition` (`Name = 100`, `Type = 20`, `DefaultValue = 500`).
...
- Ajout de `PushProjectBootstrapPipelineToGitCommandValidatorTests` et `PushProjectGeneratedArtifactsToGitCommandValidatorTests` pour verrouiller ce nouveau slice `APP-001`.
...
- Ajout de `InfraConfigAccessServiceTests` pour verrouiller l'usage du nouveau lookup infra-config read-only dans `VerifyReadAccessAsync(...)`.
- Extension de `InfrastructureConfigRepositoryTests` pour verrouiller `GetByIdReadOnlyAsync(...)` et son caractere detache.
...
- Ajout de `MultiScopeGitPushRequestBuilderTests` pour verrouiller la nouvelle extraction `APP-005` sur le merge des scopes Git et la detection de collisions de chemins.
- Ajout de `GenerateProjectBicepCommandHandlerTests` et `GenerateProjectPipelineCommandHandlerTests` pour verrouiller la garde `AmbiguousProjectLevelGeneration` en layout `MultiRepo` pendant ce refactor `APP-005`.
- Ajout de `GenerateProjectBootstrapPipelineCommandHandlerTests` et extension de `GenerateProjectPipelineCommandHandlerTests` pour verrouiller le nouveau reload enrichi unique et le retour `NotFound` si ce rechargement projet echoue apres l'autorisation.
...
- Mise a jour de audits/triage-2026-05-12.md : #164, #165, #201 et #207 restent en FIXED-IN-PR, et #166 passe maintenant en FIXED-IN-PR avec cloture fonctionnelle du sweep DB-001 jusque sur `ParameterDefinition`.
...
- Mise a jour de audits/triage-2026-05-12.md : #178 reste ouvert mais documente maintenant plusieurs remediations sures no-tracking (`ProjectAccessService.VerifyReadAccessAsync(...)`, `InfraConfigAccessService.VerifyReadAccessAsync(...)`, queries `ResourceGroup`, puis checks read-only KeyVault / RedisCache / StorageAccount) sans casser les flows de commande restes trackes par design.
- Mise a jour de audits/triage-2026-05-12.md : #168 reste ouvert mais documente maintenant un nouveau slice `APP-001` sur les validators manquants des commandes projet `PushProjectBootstrapPipelineToGit` et `PushProjectGeneratedArtifactsToGit`.
...
- Mise a jour de audits/triage-2026-05-12.md : #185 reste ouvert mais documente maintenant cinq tranches a levier (`IAppPipelineRequestFactory`, `BlobDownloadHelper`, la reutilisation du `Project` deja autorise pour la garde `CanGenerateAllFromProjectLevel()`, le reload enrichi unique `GetByIdWithAllAndPipelineVariableGroupsAsync(...)`, puis `MultiScopeGitPushRequestBuilder`) avant d'attaquer les autres god handlers.
...
- Memoire projet mise a jour avec la politique explicite de creation self-service (`Unauthorized` au lieu de `500` si `CurrentUser` manque), les slices `APP-005` via `BlobDownloadHelper` puis `MultiScopeGitPushRequestBuilder`, et la suppression du warning `NU1904` via le pin `DataProtection`.
...
- 20260512140453_AddParameterDefinitionLengthConstraints
...
- [x] dotnet test .\InfraFlowSculptor.slnx (2031 total, 0 failed, 9 skipped)
- [x] Tests cibles verts pour GeneratedFilePathValidationTests, ValidationBehaviorTests, EnumValidationTests, CoreStringLengthConfigurationTests, IndexCoverageConfigurationTests, DeleteBehaviorConfigurationTests, CreateProjectCommandValidatorTests, CreateProjectRequestTests, CreateInfrastructureConfigCommandValidatorTests, CreateInfrastructureConfigRequestTests, CreateResourceGroupCommandValidatorTests, CreateResourceGroupRequestTests, `PushProjectBootstrapPipelineToGitCommandValidatorTests`, `PushProjectGeneratedArtifactsToGitCommandValidatorTests`, ListCrossConfigReferencesQueryHandlerTests, `ProjectAccessServiceTests`, `InfraConfigAccessServiceTests`, `ProjectRepositoryTests`, `InfrastructureConfigRepositoryTests`, `GetResourceGroupQueryHandlerTests`, `ListResourceGroupResourcesQueryHandlerTests`, `ResourceGroupRepositoryTests`, les nouveaux tests DB-003 KeyVault / Redis / Storage, `GeneratePipelineCommandHandlerTests`, `DownloadPipelineCommandHandlerTests`, `AppPipelineRequestFactoryTests`, `MultiScopeGitPushRequestBuilderTests`, et le test repository cible de `GetConfigSummariesByIdsAsync(...)`
