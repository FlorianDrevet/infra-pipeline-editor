...
| 2026-03-19 | copilot | Aspire debugging note: MCP detects the running AppHost correctly once it is fully started. A frequent local pitfall is the Angular Aspire resource failing silently because port `4200` is already occupied by a manually started `npm run start`. In that case, Aspire logs show `angular-frontend` failed with `Port 4200 is already in use`, while the manually started frontend keeps using `environment.development.ts` (`http://localhost:8080`) instead of the Aspire proxy configuration (`/api-proxy`). |
| 2026-03-19 | copilot | Frontend port convention: keep Aspire frontend on `4200` and move manual Angular development serve to `4201` via `angular.json`. When changing the manual dev port, also update `src/Front/src/environments/environment.development.ts` MSAL `redirectUri` to match. |
| 2026-03-19 | copilot | Aspire AppHost frontend binding: for `AddJavaScriptApp("angular-frontend", ...)`, using `.WithExternalHttpEndpoints()` produces a random external host port even if `targetPort: 4200` is set. The supported fixed-port pattern for a JavaScript executable is `.WithHttpEndpoint(port: 4200, targetPort: 4200, env: "NG_PORT", isProxied: false)`. Keeping `isProxied: true` with identical `Port` and `TargetPort` crashes Aspire startup for non-container resources. |
| 2026-03-19 | copilot | MSAL redirect hardening: use runtime origin (`globalThis.location.origin`) in `src/Front/src/app/shared/configs/msal.config.ts` so sign-in redirect returns to the currently opened frontend host/port (e.g., `4200` under Aspire, `4201` in manual dev), avoiding accidental cross-port redirects after authentication. |
