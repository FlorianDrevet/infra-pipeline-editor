...
- **Mixed mode**: when resources of the same ARM type use DIFFERENT identity kinds (e.g. one ContainerApp with SystemAssigned and another with UserAssigned), the engine injects a **parameterized** identity block. The module receives `param identityType string` and optional `param userAssignedIdentity*Id string` params. The identity section uses a conditional expression: `identity: identityType != 'None' ? { type: identityType, userAssignedIdentities: contains(identityType, 'UserAssigned') ? userAssignedIdentityMap : null } : null`. The `principalId` output uses `contains(identityType, 'SystemAssigned') ? resource.identity.principalId : ''`. Each module instance in `main.bicep` passes its specific `identityType` value and relevant UAI resource IDs.
...
| 2026-03-28 | copilot | **Parameterized identity for mixed-mode Bicep modules** — When multiple resources of the same ARM type need different identity kinds (e.g. one ContainerApp with SystemAssigned and another with UserAssigned), the module template now uses parameterized identity (`param identityType string`, optional `param userAssignedIdentity*Id string`). If all instances use the same kind, identity stays hardcoded (existing behavior). **Files**: `GeneratedTypeModule.cs` (added `IdentityKind`, `UsesParameterizedIdentity` props), `BicepGenerationEngine.cs` (new `ComputeIdentityKindsByArmType()` + `InjectParameterizedIdentity()` methods, refactored Generate loop to detect mixed vs uniform identity per ARM type), `BicepAssembler.cs` (module declarations pass `identityType` param + UAI IDs when `UsesParameterizedIdentity` is true). No DB migration needed. |
