...
| 2026-03-19 | copilot | Aspire AppHost frontend binding: for `AddJavaScriptApp("angular-frontend", ...)`, using `.WithExternalHttpEndpoints()` produces a random external host port even if `targetPort: 4200` is set. To force the browser-facing URL to stay on `4200`, configure `.WithHttpEndpoint(port: 4200, targetPort: 4200, env: "NG_PORT")` and remove `.WithExternalHttpEndpoints()` from the frontend resource. |
