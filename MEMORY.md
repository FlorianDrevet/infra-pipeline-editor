...
│   ├── InfraFlowSculptor.PipelineGeneration  Azure DevOps pipeline YAML generation engine (stub)
...
**⚠️ IMPORTANT EF Core pitfall (Contains):** Never use `guidList.Contains(es.TypedId.Value)` in LINQ-to-EF queries. EF Core cannot translate `.Value` on a value object inside `.Contains()` against an in-memory `List<Guid>`. Instead, use a server-side subquery: build an `IQueryable<AzureResourceId>` from the DB and use `.Contains(es.TypedId)` with the typed ID directly. Example: `var idsQuery = Context.Set<AzureResource>().Where(...).Select(r => r.Id); query.Where(es => idsQuery.Contains(es.KeyVaultId))`. This translates to an efficient SQL `IN (SELECT ...)` subquery.
```
...
| 2026-05-28 | copilot | **Azure DevOps pipeline YAML generation scaffolding** — full-stack architecture for pipeline generation alongside existing Bicep generation. **New projects**: (1) `InfraFlowSculptor.GenerationCore` — shared generation models extracted from BicepGeneration: `IGenerationResult` interface, `GenerationRequest`, `EnvironmentDefinition`, `ResourceDefinition`, `ResourceGroupDefinition`, `NamingContext`, `RoleAssignmentDefinition`, `AppSettingDefinition`, `ExistingResourceReference` (8 models). BicepGeneration now references GenerationCore via ProjectReference + `GlobalUsings.cs` for seamless namespace bridging. `GenerationResult` implements `IGenerationResult` with computed `Files` property. (2) `InfraFlowSculptor.PipelineGeneration` — self-contained pipeline YAML engine (stub) with `PipelineGenerationEngine.Generate()` producing minimal `azure-pipelines.yml`, `PipelineGenerationResult` implementing `IGenerationResult`, `PipelineGenerationOptions`, `IResourceTypePipelineGenerator` strategy interface + `PipelineResourceSteps` DTO. **Shared service**: `IGeneratedArtifactService` + `GeneratedArtifactService` — factorized Download/GetFileContent/LatestFiles/Upload logic parameterized by artifact type. Blob storage pattern: `{artifactType}/{configId}/{timestamp}/...` (e.g. `pipeline/{configId}/20260328120000/azure-pipelines.yml`). Registered as singleton in Infrastructure DI. **Application**: 4 CQRS command/query pairs — `GeneratePipelineCommand` (builds GenerationRequest, calls PipelineGenerationEngine, uploads via IGeneratedArtifactService), `DownloadPipelineCommand` (delegates to IGeneratedArtifactService), `GetPipelineFileContentQuery` (reads single file), `PushPipelineToGitCommand` (mirrors PushBicepToGit pattern). `PipelineGenerationEngine` registered as singleton. **Contracts**: `GeneratePipelineRequest`/`Response`, `PushPipelineToGitRequest`/`Response`. **API**: `PipelineGenerationController` with 4 endpoints (POST `/generate-pipeline`, GET `/{configId}/download`, GET `/{configId}/files/{*filePath}`, POST `/{configId}/push-to-git`), registered in `Program.cs`. **Frontend**: `pipeline-generator.interface.ts` + `PipelineGeneratorService` (generate/download/getFileContent/pushToGit). Config-detail updated with `mat-tab-group` containing Bicep and Pipeline tabs with i18n (`GENERATION.*`, `PIPELINE.*` in FR/EN). Build: 0 .NET errors, 0 TypeScript errors. |
| 2026-03-28 | copilot | **Fixed LINQ translation error in `GetConfiguredEnvironmentsByResourceGroupAsync`** — `resourceIds.Contains(es.KeyVaultId.Value)` failed because EF Core cannot translate `.Value` access on a value object inside `.Contains()` against an in-memory `List<Guid>`. Replaced all 15 `.Contains(es.TypedId.Value)` calls with a server-side subquery: `var resourceIdsQuery = Context.Set<AzureResource>().Where(...).Select(r => r.Id)` then `idsQuery.Contains(es.TypedId)`, translating to SQL `IN (SELECT ...)`. Also made migration `RevertUniqueIndexResourceGroupIdName` fully idempotent (`DROP INDEX IF EXISTS` + `CREATE INDEX IF NOT EXISTS`). |
