...
- **Common (shared):** `ShortName`, `Prefix`, `Suffix`, `SubscriptionId`, `Order`, `RequiresApproval` — in `Domain/Common/ValueObjects`, used by Project environments
...
- `BicepGenerationEngine.InjectUserAssignedIdentity()`: regex-based post-processing that injects `identity: { type: 'UserAssigned' }` with `userAssignedIdentities` map + UAI resource ID params
- `BicepGenerationEngine.InjectParameterizedIdentity()`: injects parameterized identity block when mixed identity types coexist for the same ARM resource type

Identity injection pattern ([2026-03-28] — supports uniform + mixed modes):
- **Uniform mode**: when ALL resources of a given ARM type use the same identity kind (all SystemAssigned OR all UserAssigned), the engine hardcodes the identity block directly in the module template. Only one `identity: { type: '...' }` block and a `principalId` output are added.
- **Mixed mode**: when resources of the same ARM type use DIFFERENT identity kinds (e.g. one ContainerApp with SystemAssigned and another with UserAssigned), the engine injects a **parameterized** identity block. The module receives `param identityType ManagedIdentityType = 'SystemAssigned'` (strongly typed union from `types.bicep`: `'SystemAssigned' | 'UserAssigned' | 'SystemAssigned, UserAssigned'`) and optional `param userAssignedIdentity*Id string` params. The identity block is always present (no `None` case): `identity: { type: identityType, userAssignedIdentities: contains(identityType, 'UserAssigned') ? userAssignedIdentityMap : null }`. The `principalId` output uses `contains(identityType, 'SystemAssigned') ? resource.identity.principalId : ''`. `ManagedIdentityType` is auto-appended to the module's `types.bicep` content and auto-imported into the module template. Default value is `'SystemAssigned'`. Each module instance in `main.bicep` passes its specific `identityType` value and relevant UAI resource IDs.
- `GeneratedTypeModule.IdentityKind` tracks which identity kind each resource instance needs.
- `GeneratedTypeModule.UsesParameterizedIdentity` flags whether the module template uses the parameterized approach.
- `ComputeIdentityKindsByArmType()` pre-computes the set of distinct identity kinds per ARM type to decide uniform vs mixed.
...
| 2026-03-29 | copilot | **Pipeline generation: restrict projectName + remove v0 folder level**: (1) `GenerateSharedCiTemplate` now accepts `IReadOnlyList<string> configNames` and emits a `values:` list under the `projectName` parameter so only actual config names are accepted. `MonoRepoPipelineAssembler` extracts config names from `perConfigResults.Keys` and passes them through. (2) Removed the `v0/` folder level from all generated pipeline paths: `.azuredevops/v0/pipelines/` → `.azuredevops/pipelines/`, `.azuredevops/v0/jobs/` → `.azuredevops/jobs/`, `.azuredevops/v0/steps/` → `.azuredevops/steps/`. Updated blob storage upload paths in `GenerateProjectPipelineCommandHandler`, doc comments in models/contracts, and frontend comments in `project-detail.component.ts`. |
| 2026-03-29 | copilot | **SubscriptionId in GenerationCore**: Added `SubscriptionId` to `GenerationCore.EnvironmentDefinition`, `EnvironmentDefinitionReadModel`, `InfrastructureConfigReadRepository.BuildEnvironmentList()`, and all 4 generation handlers. Enables per-environment `subscriptionId` variable in generated pipeline YAML. |
| 2026-03-28 | copilot | **Unified generation UX with conditional tab switch**: Refactored both mono-repo (project-detail) and multi-repo (config-detail) generation panels to use a single "Generate" button instead of separate Bicep/Pipeline buttons. Tab switch (Bicep/Pipeline) is now hidden by default and only appears after clicking Generate. **config-detail**: Added `generateAll()`, `generateAllLoading`, `generationPanelOpen`, `closeGenerationPanel()` computeds+methods. Replaced always-visible `mat-tab-group` with conditional `@if (generationPanelOpen())` wrapper. Removed per-tab generate buttons. Renamed header button from "Generate Bicep" to "Generate". **project-detail**: Added `projectGenerationPanelOpen` computed, `closeProjectGenerationPanel()`. Replaced two separate collapsible `<section class="bicep-panel">` blocks with a single tab-based `generation-panel` + `mat-tab-group` pattern (matching config-detail). **SCSS**: Added `.generation-panel` styles (header, title, close button) to both components. Added `.generation-tabs` + `.generation-tab-content` styles to project-detail (already existed in config-detail). **i18n**: Added `CONFIG_DETAIL.GENERATE_ALL.BUTTON/GENERATING`, `CONFIG_DETAIL.GENERATION.TITLE`, `PROJECT_DETAIL.GENERATION.TITLE/TAB_BICEP/TAB_PIPELINE` keys in EN+FR. Renamed `PROJECT_DETAIL.GENERATE_ALL.BUTTON` from "Generate All" to "Generate". |
| 2026-03-28 | copilot | **Mono-repo Pipeline Generation**: Added full pipeline generation support for MonoRepo mode (project-level), mirroring the existing mono-repo Bicep pattern. **Backend**: `GenerateProjectPipelineCommand` + Handler (builds `GenerationRequest` per config, calls `PipelineGenerationEngine.Generate()`, uploads to `pipeline/project/{projectId}/{timestamp}/{configName}/`), `GenerateProjectPipelineCommandValidator`, `DownloadProjectPipelineCommand` + Handler (ZIP archive of latest pipeline files), `GetProjectPipelineFileContentQuery` + Handler (read single file), `PushProjectPipelineToGitCommand` + Handler + Validator (push latest pipeline files to Git). **Contracts**: `GenerateProjectPipelineResponse` record. **API**: 4 new endpoints on `ProjectController`: `POST /{projectId}/generate-pipeline`, `GET /{projectId}/generate-pipeline/download`, `GET /{projectId}/generate-pipeline/files/{*filePath}`, `POST /{projectId}/push-pipeline-to-git`. **Frontend**: `GenerateProjectPipelineResponse` interface, 4 new `ProjectService` methods, pipeline signals/computed/methods in `project-detail.component.ts`, pipeline panel HTML (generate button, loading terminal, error/retry, file tree, download, push-to-git), `PushToGitDialogData.isPipeline` flag. **i18n**: `PROJECT_DETAIL.PIPELINE.*` keys in EN/FR. |
| 2026-03-28 | copilot | **Secret static app settings — merge KeyVault reference into static mode**: Removed separate "Référence KeyVault" mode from app settings dialog. Static mode now has an `isSecret` toggle. **Domain**: new `SecretValueAssignment` enum (`ViaBicepparam`, `DirectInKeyVault`) in `Domain/Common/BaseModels/ValueObjects`. `AppSetting` gains nullable `SecretValueAssignment` property + `IsSecretStatic` computed (`IsKeyVaultReference && !IsOutputReference`). `CreateKeyVaultReference`/`UpdateToKeyVaultReference` accept `SecretValueAssignment` param. `AzureResource.AddKeyVaultReferenceAppSetting` signature updated. **Infrastructure**: `AppSettingConfiguration` maps `SecretValueAssignment` as nullable string (max 64). Migration `AddSecretValueAssignmentToAppSetting`. `InfrastructureConfigReadRepository`/`InfrastructureConfigReadModel` map the field. **Application**: `AddAppSettingCommand`, `AppSettingResult`, validators, handlers all carry `SecretValueAssignment?`. **Contracts**: `AddAppSettingRequest`/`AppSettingResponse` gain `string? SecretValueAssignment`. **Mapster**: enum↔string mapping in `AppSettingMappingConfig`. **BicepGeneration**: `AppSettingDefinition` gains `string? SecretValueAssignment`. `BicepAssembler` emits `@secure() param` + `kvSecret` module + `param = ''` in `.bicepparam` for `ViaBicepparam`; `DirectInKeyVault` uses existing inline `@Microsoft.KeyVault(SecretUri=...)` with nothing in params. **Frontend**: dialog changed from 3 modes (`static\|output\|keyvault`) to 2 modes (`static\|output`). New `isSecret` signal toggle, `secretAssignmentMode` signal. When secret: shows KV picker, RBAC check, assignment mode radio, secret name input. Removed `MODE_KEYVAULT` i18n keys, added `SECRET_TOGGLE`, `SECRET_ASSIGNMENT_*` keys (EN/FR). `resource-edit.component.html` shows `secretValueAssignment` badge. |
| 2026-03-28 | copilot | **Static app settings scoped per environment + Bicep param injection**: Replaced `AppSetting.StaticValue` (single string) with `AppSettingEnvironmentValue` child entity (one value per environment). **Domain**: new `AppSettingEnvironmentValue` entity + `AppSettingEnvironmentValueId` VO. `AppSetting.CreateStatic()`/`UpdateToStatic()` now take `IReadOnlyDictionary<string, string> environmentValues`. `AzureResource.AddStaticAppSetting()`/`UpdateAppSettingToStatic()` signatures updated. **Infrastructure**: `AppSettingEnvironmentValueConfiguration` (table `AppSettingEnvironmentValues`, unique index on AppSettingId+EnvironmentName), `AppSettingConfiguration` updated with `HasMany→EnvironmentValues`, `AzureResourceBaseRepository.GetByIdWithAppSettingsAsync()` includes `ThenInclude(EnvironmentValues)`, `InfrastructureConfigReadRepository` maps `EnvironmentValues` to dictionary. Migration `AddAppSettingEnvironmentValues` (drops `StaticValue` column, creates new table). **Application**: `AppSettingResult`, `AddAppSettingCommand`, read models all use `IReadOnlyDictionary<string, string>? EnvironmentValues` instead of `string? StaticValue`. Validators check `EnvironmentValues.Count > 0` for static mode. **Contracts**: `AddAppSettingRequest`/`AppSettingResponse` use `Dictionary<string, string>? EnvironmentValues`. **API**: Mapster mapping updated. **BicepGeneration**: `AppSettingDefinition` gains `EnvironmentValues` dictionary. `BicepAssembler` emits `param {name} string` declarations in `main.bicep`, `param {name} = '{value}'` per-env in `.bicepparam`, and uses param references in module calls. Naming: `{moduleName}{PascalCase(settingName)}`. **Frontend**: interfaces, dialog, resource-edit display, i18n (EN/FR) all updated — static mode now shows one input per environment. |
| 2026-03-28 | copilot | **Fix DirectInKeyVault ContainerApp env var format**: `BicepAssembler` had a ContainerApp-specific branch that omitted the `@Microsoft.KeyVault(SecretUri=...)` wrapper for `DirectInKeyVault` app settings, emitting bare `${kvModule.outputs.vaultUri}secrets/SECRET-NAME` instead. Removed the `isContainerApp` special case — all resource types now uniformly use `@Microsoft.KeyVault(SecretUri=...)` for DirectInKeyVault secret references. |
...
| 2026-03-28 | copilot | **Parameterized identity for mixed-mode Bicep modules** — When multiple resources of the same ARM type need different identity kinds (e.g. one ContainerApp with SystemAssigned and another with UserAssigned), the module template now uses parameterized identity (`param identityType ManagedIdentityType = 'SystemAssigned'`, strongly typed union from `types.bicep`). No `None` case — identity is always present. `ManagedIdentityType` (`'SystemAssigned' | 'UserAssigned' | 'SystemAssigned, UserAssigned'`) auto-appended to module's `types.bicep` + auto-imported. If all instances use the same kind, identity stays hardcoded (existing behavior). **Files**: `GeneratedTypeModule.cs` (added `IdentityKind`, `UsesParameterizedIdentity` props), `BicepGenerationEngine.cs` (new `ComputeIdentityKindsByArmType()`, `InjectParameterizedIdentity()`, `AddTypeImport()`, `ManagedIdentityTypeBicepType` constant; Generate loop appends type to `ModuleTypesBicepContent`), `BicepAssembler.cs` (default fallback `'SystemAssigned'` instead of `'None'`). No DB migration needed. |
| 2026-03-28 | copilot | **Unique resource name constraint + UAI ID propagation in Bicep pipeline** — (1) Added unique index `(ResourceGroupId, Name)` on `AzureResource` table — prevents duplicate resource names within the same resource group. Migration: `AddUniqueIndexResourceGroupIdName`. (2) Added `UserAssignedIdentityResourceId` (Guid?) property to `RoleAssignmentDefinition` in BicepGeneration. Both `GenerateBicepCommandHandler` and `GenerateProjectBicepCommandHandler` now propagate `UserAssignedIdentityResourceId` from the read model. (3) `BicepAssembler.GroupRoleAssignments()` now groups by `UserAssignedIdentityResourceId` (Guid) instead of `UserAssignedIdentityName` (string) — prevents incorrect merging of role assignments from different UAIs with the same name. **Frontend audit**: confirmed all UAI/role assignment lookups use GUIDs (IDs) as keys — `groupedRoleAssignments` uses `userAssignedIdentityId`, `usedByResources` uses `sourceResourceId`, `resolveIdentityName`/`resolveTargetName` use `allResources().find(r => r.id === ...)`. No frontend issues found. **Pattern**: resource names must be unique per RG (enforced by DB unique index), but always prefer ID-based correlation over name-based lookup in computational pipelines. |
| 2026-03-28 | copilot | **Reverted unique index `(ResourceGroupId, Name)` on AzureResource** — The unique index was too strict for TPT inheritance: it prevented different resource types (e.g. StorageAccount + ContainerAppEnvironment) from sharing the same name within a resource group, which is valid in Azure. Removed the index from `AzureResourceConfiguration`. Removed the `AddUniqueIndexResourceGroupIdName` migration. Added migration `RevertUniqueIndexResourceGroupIdName` that drops the unique composite index and restores the original non-unique `IX_AzureResource_ResourceGroupId` index. Also fixed pre-existing CS0452 error in `ResourceGroupRepository.GetConfiguredEnvironmentsByResourceGroupAsync()` (`.AsNoTracking()` was placed after `.Select(r => r.Id.Value)` which projects to `Guid` — moved it before the `Select`). **Pattern/Pitfall**: Never add a unique index on `(ResourceGroupId, Name)` alone in a TPT hierarchy — different resource types CAN share resource names within the same RG. If per-type uniqueness is needed, add a `ResourceTypeName` discriminator column to the base table and include it in the composite index. The UAI ID-based grouping in `BicepAssembler` (done in previous commit) is sufficient protection against name collisions in Bicep generation. |
| 2026-03-28 | copilot | **Missing environment config indicator in config-detail** — When a resource has environments defined in the project but not yet configured with per-environment settings, the config-detail resource list now shows an orange warning badge with the count of missing environments. **Backend**: `AzureResourceResult` + `AzureResourceResponse` gain `ConfiguredEnvironments` (list of environment names from TPT env settings tables). `IResourceGroupRepository.GetConfiguredEnvironmentsByResourceGroupAsync()` queries all 15 typed env settings tables (KeyVault through ServiceBusNamespace) to collect configured env names per resource. `ListResourceGroupResourcesQueryHandler` calls this alongside existing parent mapping. `ResourceGroupMappingConfig` maps the new field. UserAssignedIdentity excluded (no env settings). **Frontend**: `AzureResourceResponse` interface gains `configuredEnvironments?: string[]`. `config-detail.component.ts` adds `getMissingEnvironments()` and `hasMissingEnvironments()` helpers comparing resource's configured envs vs `projectSortedEnvironments`. Warning badge shown in all 4 resource slots (StorageAccount parent, generic parent, child, standalone). Badge shows `warning_amber` icon + count, with tooltip listing missing env names. SCSS styles: `.missing-env-badge` (orange theme: `rgba(255,152,0,0.12)` bg, `#e65100` text). i18n: `CONFIG_DETAIL.RESOURCES.MISSING_ENV_TOOLTIP` in FR/EN. **No DB migration needed.** |
| 2026-03-29 | copilot | **Unified Generate button + PipelineBasePath in Git config** — Two changes: (1) Merged separate "Generate Bicep" and "Generate Pipeline" buttons into a single "Generate All" button in `project-detail` that triggers both APIs in parallel (`Promise.all`). Both result panels (Bicep + Pipeline) still display independently. Push-to-Git buttons remain separate (one for Bicep, one for Pipeline). (2) Added `PipelineBasePath` (nullable string) to `GitRepositoryConfiguration` entity — allows separate target folder for pipeline files when pushing to Git. **Domain**: `GitRepositoryConfiguration.Create()` and `.Update()` gain `pipelineBasePath` param. `Project.SetGitRepositoryConfiguration()` updated. **Application**: `SetProjectGitConfigCommand` gains `PipelineBasePath`. `PushProjectPipelineToGitCommandHandler` + `PushPipelineToGitCommandHandler` now use `gitConfig.PipelineBasePath` instead of `gitConfig.BasePath`. `GitRepositoryConfigurationResult` gains `PipelineBasePath`. **Infrastructure**: EF config adds `PipelineBasePath` column (MaxLength 500). Migration: `AddPipelineBasePathToGitConfig` (needs API stopped to generate). **Contracts**: `SetGitConfigRequest` + `GitConfigResponse` gain `PipelineBasePath`. Mapster mapping updated. **Frontend**: `GitConfigResponse` + `SetGitConfigRequest` interfaces gain `pipelineBasePath`. `git-config-dialog` form gains `pipelineBasePath` field. `project-detail.component.html` settings tab shows `pipelineBasePath`. `project-detail.component.ts` gains `generateAll()` + `projectGenerateAllLoading` computed signal. i18n: `GENERATE_ALL.BUTTON`, `PIPELINE_BASE_PATH`, `PIPELINE_BASE_PATH_PLACEHOLDER` in FR/EN. Labels renamed: "Base path" → "Bicep base path". |
| 2026-03-29 | copilot | **Pipeline Generation Restructured — mono-repo pattern with shared templates + per-config folders**: Rewrote `PipelineGenerationEngine` to generate: (1) **Shared templates** in `.azuredevops/v0/` — `pipelines/ci.pipeline.yml` (parameterized CI template: build + publish artifacts), `jobs/deploy.job.yml` (parameterized deploy job: downloads artifacts, loops `${{ each deployment }}` calling deploy step), `steps/deploy-template.step.yml` (ARM validation + incremental deploy via `AzureResourceManagerTemplateDeployment@3` with ARM output parsing). (2) **Per-config folders** — `{ConfigName}/ci.pipeline.yml` (extends shared CI template, path trigger on config folder), `{ConfigName}/release.pipeline.yml` (per-env stages using `${{ each environment in split() }}`, references shared deploy job), `{ConfigName}/variables/vars.yml` (config-level: buildPipeline), `{ConfigName}/variables/{env}.yml` (per-env: azureResourceManagerConnection, subscriptionId, location). Created `MonoRepoPipelineAssembler` (static class, same pattern as `MonoRepoBicepAssembler`). Created `MonoRepoPipelineResult` model with `CommonFiles` + `ConfigFiles`. Updated `GenerateProjectPipelineCommandHandler` to use assembler pattern: generates per-config → assembles → uploads common files under `.azuredevops/v0/` prefix + per-config under `{configName}/` prefix. `PipelineGenerationResult` simplified (removed `MainPipelineYaml`, `Files` returns `TemplateFiles` directly). `GenerateProjectPipelineCommand`/`Result` gains `CommonFileUris`. Frontend `project.interface.ts` gains `commonFileUris`. **10 files modified/created, build 0 errors.** |
| 2026-03-29 | copilot | **Fix .azuredevops folder missing in pipeline panel**: `projectPipelineNodes` computed in `project-detail.component.ts` only processed `configFileUris` and completely ignored `commonFileUris`. Added `.azuredevops/` common folder rendering with nested `v0/pipelines/`, `v0/jobs/`, `v0/steps/` subfolders using `folder_shared` icon. Also added `variables/` subfolder nesting for per-config files (previously flattened as `variables/vars.yml` with no folder node). Tree structure now mirrors the actual generated output: `.azuredevops/ > v0/ > pipelines|jobs|steps > files` + `{ConfigName}/ > ci.pipeline.yml + release.pipeline.yml + variables/ > *.yml`. |
