...
| 2026-03-19 | copilot | **Merged origin/main into copilot/add-bicep-generation-for-assignments**: Resolved 1 conflict in InfrastructureConfigReadRepository.cs (role assignments parameter in RedisCache + StorageAccount mappings) by keeping HEAD (branch has correct role assignment support). Build validates cleanly (0 errors). Main added: Storage Account Bicep generation, list-by-resource-group queries, Entra ID-only auth redirect, frontend Bicep Generator integration, Azure documentation versioning, merge-main workflow automation. No post-merge adaptations needed (role assignments already integrated in GenerateBicepCommandHandler, BicepAssembler, read models). |
