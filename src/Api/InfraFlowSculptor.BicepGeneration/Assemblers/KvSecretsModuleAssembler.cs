...
internal static class KvSecretsModuleAssembler
{
    ...
    internal static string Generate()
    {
        ...
        sb.AppendLine();
        sb.AppendLine($"resource keyVault '{BicepArmTypeCatalog.KeyVaultArmType}' existing = {{");
        sb.AppendLine("  name: keyVaultName");
        sb.AppendLine("}");
        sb.AppendLine();
        sb.AppendLine($"resource kvSecrets '{BicepArmTypeCatalog.KeyVaultSecretsArmType}' = [for secret in secrets: {{");
