using System.ComponentModel.DataAnnotations;
using InfraFlowSculptor.Contracts.ValidationAttributes;

namespace InfraFlowSculptor.Contracts.NetworkSecurityGroups.Requests;

/// <summary>Request body for creating a new Network Security Group.</summary>
public class CreateNetworkSecurityGroupRequest : NetworkSecurityGroupRequestBase
{
    /// <summary>Unique identifier of the Resource Group that will own this NSG.</summary>
    [Required, GuidValidation]
    public required Guid ResourceGroupId { get; init; }

    /// <summary>Whether this resource already exists in Azure and is not managed by this project.</summary>
    public bool IsExisting { get; init; } = false;
}
