...
public sealed class ProjectMappingConfig : IRegister
{
    ...
    {
        // ID types
        config.NewConfig<ProjectId, Guid>().MapWith(src => src.Value);
        config.NewConfig<Guid, ProjectId>().MapWith(src => ProjectId.Create(src));
        config.NewConfig<ProjectMemberId, Guid>().MapWith(src => src.Value);
