    ...
    : AzureResourceRepository<SqlDatabase>(context), ISqlDatabaseRepository
{
    ...
    {
        return await WithSubResources(Context.Set<SqlDatabase>())
            .FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
    }

    /// <inheritdoc />
    public override async Task<SqlDatabase?> GetByIdReadOnlyAsync(
        ValueObject id,
        CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc />
    public async Task<List<SqlDatabase>> GetBySqlServerIdAsync(
        AzureResourceId sqlServerId,
        CancellationToken cancellationToken = default)
    {
        return await Context.Set<SqlDatabase>()
            .Where(x => x.SqlServerId == sqlServerId)
            .AsNoTracking()
            .ToListAsync(cancellationToken);
    }

    private static IQueryable<SqlDatabase> WithSubResources(IQueryable<SqlDatabase> query)
    {
        return query
