...
/// Inject this service into command and query handlers that require project-level authorization.
/// </summary>
public interface IProjectAccessService
{
    ...
    /// <summary>
    /// Verifies the current user is the Owner of the given project.
    /// Returns <c>NotFoundError</c> if the project does not exist or the user is not a member.
    /// Returns <c>ForbiddenError</c> if the user is not the Owner.
    /// </summary>
    Task<ErrorOr<Project>> VerifyOwnerAccessAsync(
        ProjectId projectId,
        CancellationToken cancellationToken = default);
}
