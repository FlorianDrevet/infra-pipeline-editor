...
/// <summary>Handles the <see cref="AddSubnetCommand"/> request.</summary>
public sealed class AddSubnetCommandHandler(
    IVirtualNetworkRepository virtualNetworkRepository,
    IResourceGroupRepository resourceGroupRepository,
    IInfraConfigAccessService accessService,
    IMapper mapper)
    : ICommandHandler<AddSubnetCommand, VirtualNetworkResult>
{
    /// <inheritdoc />
    public async Task<ErrorOr<VirtualNetworkResult>> Handle(
        AddSubnetCommand request,
        CancellationToken cancellationToken)
    {
        ...
        vnet.AddSubnet(name, request.AddressPrefix, delegation, request.ServiceEndpoints, privateEndpointPolicy, nsgId);
