...
internal static class AppReleasePipelineBuilder
{
    ...
    {
        var ciPipelineName = AppNamingHelper.EscapeForSingleQuotedYaml(
            AppNamingHelper.BuildCiPipelineDefinitionName(request));
        ...
        AppPipelineGenerationRequest request)
    {
        ...
        foreach (var env in request.Environments)
        {
            ...
            var variablesPath = AppHeaderEmitter.GetEnvironmentVariablesPath(envKey, request.IsMonoRepo);

            sb.AppendLine($"      - shortName: '{AppNamingHelper.EscapeForSingleQuotedYaml(envKey)}'");
            sb.AppendLine($"        name: '{AppNamingHelper.EscapeForSingleQuotedYaml(env.Name)}'");
            sb.AppendLine($"        variablesPath: '{AppNamingHelper.EscapeForSingleQuotedYaml(variablesPath)}'");
