...
public sealed class PipelineGenerationEngine
{
        ...
        string? agentPoolName = null)
    {
        ...
        var files = new Dictionary<string, string>
        {
            ["pipelines/ci.pipeline.yml"] = GenerateSharedCiTemplate(configNames, agentPoolName),
            ["pipelines/pr.pipeline.yml"] = GenerateSharedPrTemplate(configNames, agentPoolName),
    ...
    private static string GenerateSharedPrTemplate(IReadOnlyList<string> configNames, string? agentPoolName)
    {
        var sb = new StringBuilder();
        ...
        sb.AppendLine("    value: $(Build.SourcesDirectory)/${{ parameters.projectName }}");
        sb.AppendLine();
        sb.AppendLine("stages:");
        ...
        sb.AppendLine();
        sb.AppendLine("          - pwsh: |");
        sb.AppendLine("              $configPath = \"$(Build.SourcesDirectory)/bicepconfig.json\"");
    ...
    private static string GenerateSharedCheckoutStep()
    {
        ...
        sb.AppendLine("        Write-Host \"Checking out $dir\"");
        sb.AppendLine("        git sparse-checkout add $dir");
        sb.AppendLine("      }");
        sb.AppendLine();
        sb.AppendLine("      git -c http.extraheader=\"AUTHORIZATION: bearer $(System.AccessToken)\" `");
        sb.AppendLine("          pull origin \"$(Build.SourceBranch)\"");
        sb.AppendLine();
        sb.AppendLine("      Get-ChildItem -Directory -Force | ForEach-Object { Write-Host $_.Name }");
        sb.AppendLine();
        sb.AppendLine("    displayName: \"Sparse Checkout\"");
        sb.AppendLine("    condition: ${{ gt(length(parameters.directories), 0) }}");
        sb.AppendLine();
        sb.AppendLine("  - checkout: self");
        sb.AppendLine("    clean: true");
        sb.AppendLine("    displayName: \"Full Checkout\"");
        sb.AppendLine("    condition: ${{ eq(length(parameters.directories), 0) }}");

        return sb.ToString();
    }

    private static string GenerateSharedCiTemplate(IReadOnlyList<string> configNames, string? agentPoolName)
    {
        var sb = new StringBuilder();
