...
**Orchestration :** par défaut, orchestre directement (lis la mémoire, délègue aux sous-agents, charge les skills) — tu es le thread principal. La commande **`/dev`** est **optionnelle** : c'est le protocole « artillerie lourde » pour les tâches complexes / cross-cutting / planifiées (passe de contradiction Bicep, phase Research Graphify, scratchpad multi-agents, tracker multi-PC). Pour une tâche simple et ciblée, pas besoin de `/dev`.
...
| `graphify-corpus` | Exploration du code et du corpus, contexte architectural, validation post-change |
...
10. **Graphify :** avant de modifier un symbole partagé, lancer une requête `query` ciblée puis confirmer les dépendances par lecture et validation exécutable.
11. **Graphe unique :** Graphify couvre le code, la documentation, les diagrammes et les audits. Utiliser les tests, le build et `git diff` pour confirmer l'impact réel d'un changement.
...
`graphify` (knowledge graph du code, de la documentation, des audits et des diagrammes) et `sonarqube` (qualité). Graphify est exposé via `.vscode/mcp.json`; son graphe se rafraîchit avec `python -m graphify update .`.
