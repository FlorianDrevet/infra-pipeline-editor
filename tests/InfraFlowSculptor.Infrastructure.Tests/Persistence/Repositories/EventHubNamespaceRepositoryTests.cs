...
public sealed class EventHubNamespaceRepositoryTests : IDisposable
{
    private const string DefaultName = "ehn-shared";
    private const string OtherName = "ehn-other";

    private readonly ProjectDbContext _context;
    private readonly EventHubNamespaceRepository _sut;

    public EventHubNamespaceRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new EventHubNamespaceRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static EventHubNamespace NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
        => EventHubNamespace.Create(
            resourceGroupId,
            new Name(name),
    ...
    public async Task Given_StoredEntity_When_GetByIdAsync_Then_ReturnsEntity_Async()
    {
        ...
        _context.EventHubNamespaces.Add(entity);
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.EventHubNamespaces.AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
