...
public sealed class ServiceBusNamespaceRepositoryTests : IDisposable
{
    private const string DefaultName = "sb-shared";
    private const string OtherName = "sb-other";

    private readonly ProjectDbContext _context;
    private readonly ServiceBusNamespaceRepository _sut;

    public ServiceBusNamespaceRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new ServiceBusNamespaceRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static ServiceBusNamespace NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
        => ServiceBusNamespace.Create(
            resourceGroupId,
            new Name(name),
    ...
    public async Task Given_StoredEntity_When_GetByIdAsync_Then_ReturnsEntity_Async()
    {
        ...
        _context.Set<ServiceBusNamespace>().Add(entity);
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.Set<ServiceBusNamespace>().AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
