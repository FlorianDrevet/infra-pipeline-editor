...
public sealed class ContainerAppEnvironmentRepositoryTests : IDisposable
{
    private const string DefaultName = "cae-shared";
    private const string OtherName = "cae-other";

    private readonly ProjectDbContext _context;
    private readonly ContainerAppEnvironmentRepository _sut;

    public ContainerAppEnvironmentRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new ContainerAppEnvironmentRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static ContainerAppEnvironment NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
        => ContainerAppEnvironment.Create(
            resourceGroupId,
            new Name(name),
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.ContainerAppEnvironments.AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
