...
public sealed class AppServicePlanRepositoryTests : IDisposable
{
    private const string DefaultName = "asp-shared";
    private const string OtherName = "asp-other";

    private readonly ProjectDbContext _context;
    private readonly AppServicePlanRepository _sut;

    public AppServicePlanRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new AppServicePlanRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static AppServicePlan NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.AppServicePlans.AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
