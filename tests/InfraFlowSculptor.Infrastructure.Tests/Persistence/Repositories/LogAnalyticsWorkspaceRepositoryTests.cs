...
public sealed class LogAnalyticsWorkspaceRepositoryTests : IDisposable
{
    private const string DefaultName = "law-shared";
    private const string OtherName = "law-other";

    private readonly ProjectDbContext _context;
    private readonly LogAnalyticsWorkspaceRepository _sut;

    public LogAnalyticsWorkspaceRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new LogAnalyticsWorkspaceRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static LogAnalyticsWorkspace NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
        => LogAnalyticsWorkspace.Create(
            resourceGroupId,
            new Name(name),
    ...
    public async Task Given_StoredEntity_When_GetByIdAsync_Then_ReturnsEntity_Async()
    {
        ...
        _context.Set<LogAnalyticsWorkspace>().Add(entity);
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.Set<LogAnalyticsWorkspace>().AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
