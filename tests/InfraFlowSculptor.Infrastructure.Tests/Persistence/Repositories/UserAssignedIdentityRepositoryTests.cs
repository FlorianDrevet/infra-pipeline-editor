...
public sealed class UserAssignedIdentityRepositoryTests : IDisposable
{
    private const string DefaultName = "id-shared";
    private const string OtherName = "id-other";

    private readonly ProjectDbContext _context;
    private readonly UserAssignedIdentityRepository _sut;

    public UserAssignedIdentityRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new UserAssignedIdentityRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static UserAssignedIdentity NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
        => UserAssignedIdentity.Create(
            resourceGroupId,
            new Name(name),
    ...
    public async Task Given_StoredEntity_When_GetByIdAsync_Then_ReturnsEntity_Async()
    {
        ...
        _context.Set<UserAssignedIdentity>().Add(entity);
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.Set<UserAssignedIdentity>().AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
