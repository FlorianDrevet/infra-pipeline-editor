...
public sealed class VirtualNetworkRepositoryTests : IDisposable
{
    private const string DefaultName = "vnet-shared";
    private const string OtherName = "vnet-other";

    private readonly ProjectDbContext _context;
    private readonly VirtualNetworkRepository _sut;

    public VirtualNetworkRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new VirtualNetworkRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static VirtualNetwork NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.Set<VirtualNetwork>().AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
    ...
    [Fact]
    public async Task Given_StoredSubnet_When_SubnetExistsAsync_Then_ReturnsTrue_Async()
    {
        // Arrange
        var entity = NewEntity(ResourceGroupId.CreateUnique());
        ...
        // Act
        var exists = await _sut.SubnetExistsAsync(new AzureResourceId(subnet.Id.Value));
    ...
    [Fact]
    public async Task Given_UnknownSubnetId_When_SubnetExistsAsync_Then_ReturnsFalse_Async()
    {
        // Act
        var exists = await _sut.SubnetExistsAsync(AzureResourceId.CreateUnique());
