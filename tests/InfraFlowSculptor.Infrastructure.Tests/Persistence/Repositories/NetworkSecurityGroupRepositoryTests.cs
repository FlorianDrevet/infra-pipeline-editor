...
public sealed class NetworkSecurityGroupRepositoryTests : IDisposable
{
    private const string DefaultName = "nsg-shared";
    private const string OtherName = "nsg-other";

    private readonly ProjectDbContext _context;
    private readonly NetworkSecurityGroupRepository _sut;

    public NetworkSecurityGroupRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new NetworkSecurityGroupRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static NetworkSecurityGroup NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
    ...
    public async Task Given_StoredEntity_When_GetByIdAsync_Then_ReturnsEntity_Async()
    {
        ...
        _context.Set<NetworkSecurityGroup>().Add(entity);
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.Set<NetworkSecurityGroup>().AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
