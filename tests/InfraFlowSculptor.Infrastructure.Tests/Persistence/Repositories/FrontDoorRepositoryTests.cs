...
public sealed class FrontDoorRepositoryTests : IDisposable
{
    private const string DefaultName = "fd-shared";
    private const string OtherName = "fd-other";

    private readonly ProjectDbContext _context;
    private readonly FrontDoorRepository _sut;

    public FrontDoorRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new FrontDoorRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static FrontDoor NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
        => FrontDoor.Create(
            resourceGroupId,
            new Name(name),
    ...
    public async Task Given_StoredEntity_When_GetByIdAsync_Then_ReturnsEntity_Async()
    {
        ...
        _context.Set<FrontDoor>().Add(entity);
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.Set<FrontDoor>().AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
