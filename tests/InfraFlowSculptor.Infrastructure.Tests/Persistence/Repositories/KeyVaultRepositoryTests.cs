...
public sealed class KeyVaultRepositoryTests : IDisposable
{
    private const string DefaultName = "kv-shared";
    private const string OtherName = "kv-other";

    private readonly ProjectDbContext _context;
    private readonly KeyVaultRepository _sut;

    public KeyVaultRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new KeyVaultRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static KeyVault NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
        => KeyVault.Create(
            resourceGroupId,
            new Name(name),
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.KeyVaults.AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
