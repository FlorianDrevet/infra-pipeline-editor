...
public sealed class ApplicationInsightsRepositoryTests : IDisposable
{
    private const string DefaultName = "ai-shared";
    private const string OtherName = "ai-other";

    private readonly ProjectDbContext _context;
    private readonly ApplicationInsightsRepository _sut;

    public ApplicationInsightsRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new ApplicationInsightsRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static ApplicationInsights NewEntity(
        ...
        => ApplicationInsights.Create(
            ...
            logAnalyticsWorkspaceId ?? AzureResourceId.CreateUnique());
    ...
    public async Task Given_StoredEntity_When_GetByIdAsync_Then_ReturnsEntity_Async()
    {
        ...
        _context.ApplicationInsightsResources.Add(entity);
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.ApplicationInsightsResources.AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
    ...
    [Fact]
    public async Task Given_StoredEntities_When_GetByLogAnalyticsWorkspaceIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        // Arrange
        var lawId = AzureResourceId.CreateUnique();
        var matching = NewEntity(ResourceGroupId.CreateUnique(), logAnalyticsWorkspaceId: lawId);
        var unrelated = NewEntity(ResourceGroupId.CreateUnique(), name: OtherName);
        _context.ApplicationInsightsResources.AddRange(matching, unrelated);
        await _context.SaveChangesAsync();

        // Act
        var result = await _sut.GetByLogAnalyticsWorkspaceIdAsync(lawId);
