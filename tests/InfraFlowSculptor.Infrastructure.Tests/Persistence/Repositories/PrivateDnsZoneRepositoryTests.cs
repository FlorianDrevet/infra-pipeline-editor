...
public sealed class PrivateDnsZoneRepositoryTests : IDisposable
{
    private const string DefaultName = "privatelink.database.windows.net";
    private const string OtherName = "privatelink.blob.core.windows.net";

    private readonly ProjectDbContext _context;
    private readonly PrivateDnsZoneRepository _sut;

    public PrivateDnsZoneRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new PrivateDnsZoneRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static PrivateDnsZone NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
        => PrivateDnsZone.Create(
            resourceGroupId,
            new Name(name),
    ...
    public async Task Given_StoredEntity_When_GetByIdAsync_Then_ReturnsEntity_Async()
    {
        ...
        _context.Set<PrivateDnsZone>().Add(entity);
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.Set<PrivateDnsZone>().AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
