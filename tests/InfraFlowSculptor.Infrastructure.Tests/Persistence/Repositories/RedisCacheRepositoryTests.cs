...
public sealed class RedisCacheRepositoryTests : IDisposable
{
    private const string DefaultName = "redis-shared";
    private const string OtherName = "redis-other";

    private readonly ProjectDbContext _context;
    private readonly RedisCacheRepository _sut;

    public RedisCacheRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new RedisCacheRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static RedisCache NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
        => RedisCache.Create(
            resourceGroupId,
            new Name(name),
    ...
    public async Task Given_StoredEntity_When_GetByIdAsync_Then_ReturnsEntity_Async()
    {
        ...
        _context.Set<RedisCache>().Add(entity);
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.Set<RedisCache>().AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
