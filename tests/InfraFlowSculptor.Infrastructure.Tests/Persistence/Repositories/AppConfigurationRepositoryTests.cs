...
public sealed class AppConfigurationRepositoryTests : IDisposable
{
    private const string DefaultName = "ac-shared";
    private const string OtherName = "ac-other";

    private readonly ProjectDbContext _context;
    private readonly AppConfigurationRepository _sut;

    public AppConfigurationRepositoryTests()
    {
        _context = InMemoryDbContextFactory.Create();
        _sut = new AppConfigurationRepository(_context);
    }

    public void Dispose() => _context.Dispose();

    private static AppConfiguration NewEntity(ResourceGroupId resourceGroupId, string name = DefaultName)
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.AppConfigurations.AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
    ...
    [Fact]
    public async Task Given_StoredEntity_When_GetByIdWithConfigurationKeysAsync_Then_ReturnsEntity_Async()
    {
        // Arrange
        var entity = NewEntity(ResourceGroupId.CreateUnique());
        _context.AppConfigurations.Add(entity);
        ...
        // Act
        var result = await _sut.GetByIdWithConfigurationKeysAsync(entity.Id);
    ...
    [Fact]
    public async Task Given_StoredEntity_When_GetByIdWithConfigurationKeysAndRoleAssignmentsAsync_Then_ReturnsEntity_Async()
    {
        // Arrange
        var entity = NewEntity(ResourceGroupId.CreateUnique());
        _context.AppConfigurations.Add(entity);
        ...
        // Act
        var result = await _sut.GetByIdWithConfigurationKeysAndRoleAssignmentsAsync(entity.Id);
