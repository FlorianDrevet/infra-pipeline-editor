...
public sealed class WebAppRepositoryTests : IDisposable
{
    private const string DefaultName = "wa-shared";
    private const string OtherName = "wa-other";
    private const string RuntimeVersion = "8.0";

    private readonly ProjectDbContext _context;
    private readonly WebAppRepository _sut;

    public WebAppRepositoryTests()
    {
    ...
    private static WebApp NewEntity(
    ...
    public async Task Given_StoredEntities_When_GetByResourceGroupIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        _context.Set<WebApp>().AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
    ...
    public async Task Given_StoredEntities_When_GetByAppServicePlanIdAsync_Then_ReturnsOnlyMatching_Async()
    {
        ...
        var rgId = ResourceGroupId.CreateUnique();
        var owned = NewEntity(rgId, planId);
        var unrelated = NewEntity(ResourceGroupId.CreateUnique(), name: OtherName);
        _context.Set<WebApp>().AddRange(owned, unrelated);
        await _context.SaveChangesAsync();
