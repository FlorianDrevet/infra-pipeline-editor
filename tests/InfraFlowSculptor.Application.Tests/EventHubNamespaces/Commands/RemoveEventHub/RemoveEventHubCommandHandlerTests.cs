...
public sealed class RemoveEventHubCommandHandlerTests
{
    ...
    private readonly RemoveEventHubCommand _command;
    private readonly RemoveEventHubCommandHandler _sut;

    public RemoveEventHubCommandHandlerTests()
    {
        ...
        _eventHubNamespace = EventHubNamespace.Create(
            ...
            new Name("ehn-shared"),
            new Location(Location.LocationEnum.FranceCentral));
        _command = new RemoveEventHubCommand(
            _eventHubNamespace.Id,
            Guid.NewGuid());
        _sut = new RemoveEventHubCommandHandler(
    ...
    [Fact]
    public async Task Given_EventHubNotFound_When_Handle_Then_ReturnsErrorAsync()
    {
        // Arrange — no event hub with this ID exists on the namespace
        _eventHubNamespaceRepository.GetByIdAsync(Arg.Any<AzureResourceId>(), Arg.Any<CancellationToken>())
            .Returns(_eventHubNamespace);
        _resourceGroupRepository.GetByIdAsync(Arg.Any<ValueObject>(), Arg.Any<CancellationToken>())
            .Returns(_resourceGroup);
