...
public sealed class GenerateBootstrapCommandValidatorTests
{
    private readonly GenerateBootstrapCommandValidator _sut = new();
    ...
    {
        var command = new GenerateBootstrapCommand(Guid.Empty);

        var result = _sut.Validate(command);

        result.IsValid.Should().BeFalse();
        result.Errors.Should().Contain(e => e.PropertyName == nameof(GenerateBootstrapCommand.InfrastructureConfigId));
    }
}
