...
public sealed class AddCustomDomainCommandHandlerTests
{
    private const string DomainName = "api.example.com";
    ...
    private readonly AddCustomDomainCommand _command;
    private readonly AddCustomDomainCommandHandler _sut;

    public AddCustomDomainCommandHandlerTests()
    {
        ...
        // Set ResourceType via reflection (normally set by EF Core infrastructure)
        ...
        _command = new AddCustomDomainCommand(
            _webApp.Id,
            EnvironmentName,
            DomainName);
        _sut = new AddCustomDomainCommandHandler(
    ...
    [Fact]
    public async Task Given_WriteAccessGranted_When_Handle_Then_AddsCustomDomainAndReturnsResultAsync()
    {
        // Arrange
        _azureResourceRepository.GetByIdWithCustomDomainsAsync(Arg.Any<AzureResourceId>(), Arg.Any<CancellationToken>())
            .Returns(_webApp);
        ...
        result.Value.DomainName.Should().Be(DomainName);
        result.Value.EnvironmentName.Should().Be(EnvironmentName);
