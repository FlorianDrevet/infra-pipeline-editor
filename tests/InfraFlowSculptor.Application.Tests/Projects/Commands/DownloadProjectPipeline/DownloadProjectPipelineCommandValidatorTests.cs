...
public sealed class DownloadProjectPipelineCommandValidatorTests
{
    private const string ProjectIdProperty = "ProjectId.Value";

    private readonly DownloadProjectPipelineCommandValidator _sut = new();

    [Fact]
    public void Given_ValidProjectId_When_Validate_Then_Succeeds()
    {
        // Arrange
        var command = new DownloadProjectPipelineCommand(ProjectId.CreateUnique());
    ...
    {
        // Arrange
        var command = new DownloadProjectPipelineCommand(new ProjectId(Guid.Empty));
