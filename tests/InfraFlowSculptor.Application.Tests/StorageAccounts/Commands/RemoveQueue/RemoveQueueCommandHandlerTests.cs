...
public sealed class RemoveQueueCommandHandlerTests
{
    ...
    private readonly RemoveQueueCommand _command;
    private readonly RemoveQueueCommandHandler _sut;

    public RemoveQueueCommandHandlerTests()
    {
        ...
        _command = new RemoveQueueCommand(
            _storageAccount.Id,
            StorageQueueId.CreateUnique());
        _sut = new RemoveQueueCommandHandler(
    ...
    [Fact]
    public async Task Given_QueueNotFound_When_Handle_Then_ReturnsNotFoundAsync()
    {
        // Arrange
        _storageAccountRepository.GetByIdWithSubResourcesAsync(Arg.Any<AzureResourceId>(), Arg.Any<CancellationToken>())
            .Returns(_storageAccount);
        ...
        _accessService.VerifyWriteAccessAsync(_config.Id, Arg.Any<CancellationToken>())
            .Returns(_config);
        _storageAccountRepository.RemoveQueueAsync(_storageAccount.Id, _command.QueueId)
            .Returns(false);
