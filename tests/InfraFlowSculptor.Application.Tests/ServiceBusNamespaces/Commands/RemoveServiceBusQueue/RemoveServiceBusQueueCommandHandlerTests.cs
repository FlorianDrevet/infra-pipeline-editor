...
public sealed class RemoveServiceBusQueueCommandHandlerTests
{
    ...
    private readonly RemoveServiceBusQueueCommand _command;
    private readonly RemoveServiceBusQueueCommandHandler _sut;

    public RemoveServiceBusQueueCommandHandlerTests()
    {
        ...
        _serviceBusNamespace = ServiceBusNamespace.Create(
            ...
            new Name("sb-shared"),
            new Location(Location.LocationEnum.FranceCentral));
        _command = new RemoveServiceBusQueueCommand(
            _serviceBusNamespace.Id,
            Guid.NewGuid());
        _sut = new RemoveServiceBusQueueCommandHandler(
    ...
    [Fact]
    public async Task Given_QueueNotFound_When_Handle_Then_ReturnsErrorAsync()
    {
        // Arrange — no queue with this ID
        _serviceBusNamespaceRepository.GetByIdAsync(Arg.Any<AzureResourceId>(), Arg.Any<CancellationToken>())
            .Returns(_serviceBusNamespace);
        _resourceGroupRepository.GetByIdAsync(Arg.Any<ValueObject>(), Arg.Any<CancellationToken>())
            .Returns(_resourceGroup);
