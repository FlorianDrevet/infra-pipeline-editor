...
- **Graphe :** `graphify-out/graph.json` et `graphify-out/GRAPH_REPORT.md`.
...
- **Règle pratique :** utiliser `python -m graphify query "concept" --graph .\graphify-out\graph.json`, puis `explain` ou `path` pour approfondir les relations.
