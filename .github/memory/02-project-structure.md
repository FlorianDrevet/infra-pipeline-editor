...
- Conversational draft parsing now also extracts `ApplicationStack` keywords for compute resources before clarification, while ARM import preview returns `SuggestedApplicationStack` for mapped WebApp/FunctionApp/ContainerApp resources by inspecting `siteConfig.linuxFxVersion` and container image metadata [2026-05-21].
