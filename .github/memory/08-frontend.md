...
- **Import App Settings from JSON [2026-05-11]:** `ImportAppSettingsDialogComponent` in `resource-edit/import-app-settings-dialog/` allows bulk import of env vars from `appsettings.json`. Three-step flow: (1) upload/drag-drop or paste JSON, (2) board of flattened keys with per-variable source selection (static/output/pipeline variable group/skip), (3) sequential API import with progress bar. Uses existing `POST /azure-resources/{id}/app-settings` endpoint per entry. Reuses `ProjectService.getPipelineVariableGroups()` and `AzureResourceService.getOutputDefinitions()`. Button wired next to "Add" in resource-edit app settings tab. i18n keys under `RESOURCE_EDIT.IMPORT_APP_SETTINGS.*`.
