...
- **Generated file explorer ordering [2026-05-11]:** `project-detail-tree.helpers.ts` and the duplicated tree builders inside `SplitGenerationSwitcherComponent` must sort hierarchical Bicep/Pipeline entry paths before flattening them into `BicepTreeNode[]`. Without deterministic ordering, a later file can render visually under a sibling folder even when its backend path and `parentFolderKey` are correct.
