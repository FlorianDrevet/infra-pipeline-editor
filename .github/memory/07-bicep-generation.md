...
- **ARM/API catalog centralization [2026-05-12]:** shared ARM API versions and qualified resource-type strings now live in `BicepGeneration/Constants/BicepArmTypeCatalog.cs`. Cross-file consumers (`ResourceTypeMetadata`, `RoleAssignmentModuleTemplates`, `MainBicepAssembler`, `KvSecretsModuleAssembler`) and the remaining legacy raw module templates must route through that catalog. Keep generator-local aliases only for readability; the source of truth is the catalog.
- **Generation error boundary note [2026-05-12]:** `GEN-006` stays open, but the decision is now framed: do not push `ErrorOr<>` through every Bicep stage/generator. Normalize expected generation failures at the engine boundary, and keep invariant/programmer faults as exceptions. The branch note lives in `docs/architecture/generation-error-handling.md`.
- **Engine-boundary error normalization [2026-05-12]:** `BicepGenerationEngine`, `PipelineGenerationEngine`, and `AppPipelineGenerationEngine` now return `ErrorOr<T>` for known expected generation failures via `GenerationCore/Errors/GenerationErrors.cs`. Internal stages/helpers still throw; the engines classify only expected failures (unsupported/missing generators, invalid deployment mode, invalid pipeline variable-group names, invalid Key Vault secret names) and let invariant/programmer faults continue to surface as exceptions. `ConfigPipelineGenerationService` plus the four generation command handlers now propagate these typed engine errors instead of letting expected generation failures fall through to the API global `500` handler.
...
- **Output-pruning re-audit [2026-05-12]:** `[AUDIT][GEN-002]` is now obsolete on the current branch. The executable pruning flow is IR-based (`IrOutputPruningStage`, `IrMonoRepoOutputPruner`, `OutputUsageTracker`) and no longer relies on post-hoc `string.Insert()` / substring matching.
...
- **Derived ACR login server wiring [2026-05-12]:** compute modules that expose `acrLoginServer` must not leave it as a top-level `main.bicep` / `.bicepparam` placeholder when `containerRegistryId` is known. `ParentReferenceResolutionStage` now resolves that FK either to a parent module output reference (`loginServer`) for in-config registries or to an existing-resource property reference (`properties.loginServer`) for cross-config registries, and `MainBicepAssembler` / `ParameterFileAssembler` must treat those derived references as non-user parameters. Keep the regression locked by `ParentReferenceResolutionStageTests` and `BicepAssemblerTests`.
