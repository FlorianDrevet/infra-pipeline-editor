...
- `PushProjectBootstrapPipelineToGitCommandHandler` filters by bucket prefix in `GetLatestBootstrapFilesAsync(projectId, bucketPrefix, ct)`. In SplitInfraCode it pushes only the `infra/` bucket to the infra repo. `PushProjectArtifactsToMultiRepoCommandHandler` adds a parallel `LoadLatestBootstrapFilesSplitAsync` and includes the `app/` bootstrap in the code repo push scope.
- Frontend `SplitGenerationSwitcherComponent` exposes `infraBootstrapNodes`/`appBootstrapNodes` (computed from the union `fileUris` by bucket prefix) and renders a Bootstrap tree under both Infra and Code outer tabs (`PROJECT_DETAIL.GENERATION.TAB_BOOTSTRAP_APP`). `Push code` is unlocked when an app bootstrap or app pipeline is present. New i18n keys: `SWITCHER.BOOTSTRAP_APP_INFO` (EN/FR), `BOOTSTRAP_INFO`/`INFRA_PUSH_DESC`/`CODE_PUSH_DESC` updated to reflect the asymmetric ownership model.
