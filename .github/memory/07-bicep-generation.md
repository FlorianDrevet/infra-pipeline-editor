...
- **Dedicated ACR pull UAI [2026-05-19]:** `acrPullIdentityId` is the dedicated identity used only for image pulls and is independent from resource-level UAIs. `ParentReferenceResolutionStage` must resolve it to the parent ACR managed-identity client id, and regression coverage lives in `ParentReferenceResolutionStageTests`.
...
- **Project snapshot refreshed from `infraDb` [2026-05-18]:** `docs/project-snapshots/fb8699ea-ifs-project.md` is back in sync with the live project config. It now reflects `LayoutPreset = SplitInfraCode`, the dual Azure DevOps repos (`infra` / `code`), both Container Apps bound to ACR `36ba74cb-c0a1-40a3-b224-e3be1c1b4b46`, backend/frontend UAIs, frontend ingress port `81`, health probes, and the pending managed-certificate domain `infraflowsculptor.fr`.
