...
| `/projects` | POST | `/{id:guid}/push-split-infra-code-artifacts-to-git` | `PushProjectArtifactsToMultiRepoCommand` (legacy SplitInfraCode infra-only, code-only, or dual push; targets are project repository ids; always 200 with per-repo `RepoPushResult`) [2026-09-01] |
| `/projects` | POST | `/{id:guid}/push-multi-repo-artifacts-to-git` | `PushProjectMultiRepoArtifactsCommand` (MultiRepo bulk push; targets are grouped by `InfrastructureConfigId` and `InfraConfigRepositoryId`; role is derived from persisted `ConfigLayoutMode`/`ContentKinds`; returns one independent result per target and does not imply cross-repository atomicity) [2026-09-01] |
