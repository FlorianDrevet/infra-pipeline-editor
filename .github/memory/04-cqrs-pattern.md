...
- APP-012 closure rule [2026-05-13]: reduce validator duplication with narrow shared rule helpers such as `TagValidationRules` and `RepositoryConnectionValidationRules`, not with a generic `EntityCommandValidator<T>` base class. If the shared logic is only a couple of reusable rule clusters, keep them as helper methods and let each validator stay explicit.
...
- `IProjectPipelineAggregator` is the APP-005 closure slice for project-level mono-repo pipeline generation: keep the per-config `BuildGenerationRequestForPipeline(...)` + infra pipeline generation + app pipeline generation + environment deduplication + `MonoRepoPipelineAssembler.Assemble(...)` flow out of `GenerateProjectPipelineCommandHandler`, and keep that service under `Projects/Common/Generation` rather than widening the shared infra-config services.
- `IMonoRepoBlobUploadOrchestrator` is the paired APP-005 closure slice for project-level artifact uploads: keep the mono-repo Bicep/pipeline blob path mapping and typed URI-map construction out of `GenerateProjectPipelineCommandHandler` and `GenerateProjectBicepCommandHandler`, but keep the timestamped prefix generation in the handlers so the service stays focused on upload orchestration only.
