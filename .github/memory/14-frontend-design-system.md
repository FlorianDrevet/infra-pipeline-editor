...
- **Project-detail config cards (2026-05-11)** : le header de `config-card` doit rester sur une seule ligne (`align-items: center`, `justify-content: space-between`) et le `h3` doit explicitement resetter ses marges par défaut. Sans ça, le nom reste au-dessus tandis que les actions passent sur une seconde ligne malgré la carte en layout horizontal.
