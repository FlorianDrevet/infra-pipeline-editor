...
- `IInfrastructureConfigRepository.GetByIdWithMembersReadOnlyAsync(...)` is the follow-up DB-003 split for pure infra-config queries that still need `CrossConfigReferences` loaded. `ListCrossConfigReferencesQueryHandler` now uses this detached path instead of the tracked `GetByIdWithMembersAsync(...)` lookup.
...
- `ListAppConfigurationKeysQueryHandler` is the next DB-003 follow-up on top of that `ResourceGroup` split: when a query only resolves `InfraConfigId` to authorize variable-group name expansion, use `IResourceGroupRepository.GetByIdReadOnlyAsync(...)` instead of the tracked lookup.
...
- The same DB-003 read-only pattern also covers the remaining simple detail queries whose repositories can rely directly on the generic `IRepository.GetByIdReadOnlyAsync(...)` / `AzureResourceRepository<TEntity>.GetByIdReadOnlyAsync(...)` path without new repository-specific overrides: `GetApplicationInsightsQueryHandler`, `GetAppConfigurationQueryHandler`, `GetUserAssignedIdentityQueryHandler`, `GetLogAnalyticsWorkspaceQueryHandler`, `GetEventHubNamespaceQueryHandler`, and `GetServiceBusNamespaceQueryHandler` now all use read-only aggregate + resource-group lookups, and direct application tests lock that they no longer call the tracked `GetByIdAsync(...)` path.
- `IAzureResourceRepository` / `AzureResourceBaseRepository` now expose no-tracking variants for the polymorphic base-resource read paths that cannot use `AzureResourceRepository<TEntity>` directly: `GetByIdReadOnlyAsync(...)`, `GetByIdWithRoleAssignmentsReadOnlyAsync(...)`, and `GetByIdWithRoleAssignmentsAndAppSettingsReadOnlyAsync(...)` mirror the tracked include graph with `AsNoTracking()` so pure RBAC queries can stay detached even when they need polymorphic `AzureResource` resolution.
- The DB-003 sweep now also covers the RBAC read-query family built on top of that polymorphic base repository: `ListAvailableRoleDefinitionsQueryHandler`, `ListRoleAssignmentsQueryHandler`, `ListRoleAssignmentsByIdentityQueryHandler`, and `AnalyzeRoleAssignmentImpactQueryHandler` all switched to the new `IAzureResourceRepository` read-only variants plus `IResourceGroupRepository.GetByIdReadOnlyAsync(...)`, with direct tests locking the no-tracking path.
