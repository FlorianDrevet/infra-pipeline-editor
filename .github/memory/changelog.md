...
- [2026-05-13] `dev` — Same-session follow-up on audit reconciliation and APP-001: closed GitHub issues `#329` (`SEC-008`), `#206` (`API-003`), `#224` (`GEN-009`), and `#243` (`[AUDIT][GEN-002]`) after branch verification, then added `RevokePersonalAccessTokenCommandValidator` plus focused validator tests. Local triage was resynced again to the post-closure GitHub state (`41` open issues, `16` true engineering items left), CQRS memory now records the minimal-validator pattern for identity-only revoke commands, and test debt now tracks the still-missing direct coverage for the create/revoke PersonalAccessToken handlers. Focused validation: `dotnet test .\tests\InfraFlowSculptor.Application.Tests\InfraFlowSculptor.Application.Tests.csproj --no-restore --filter "FullyQualifiedName~RevokePersonalAccessTokenCommandValidatorTests"` (`2` passed).
