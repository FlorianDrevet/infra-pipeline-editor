...
| Date | Author | Change |
|------|--------|--------|
| 2026-04-04 | copilot | Fix generated Bicep parameter files for Azure DevOps release validation: `ParameterFileAssembler` now emits `using '../main.bicep'` because `main.<env>.bicepparam` files live under `parameters/`. The previous `using 'main.bicep'` caused ARM deployment tasks to find and compile `main.bicep` but then fail resolving the parameter file relationship with `Could not find any file matching the template file pattern`. |
| 2026-04-04 | copilot | Fix infra release template file resolution after subscription-scope migration: generated deployment entries now carry exact `$(Build.SourcesDirectory)/...` paths for `main.bicep` and `main.<env>.bicepparam`, based on Git `BasePath`, instead of wildcard `$(Pipeline.Workspace)/**/...` patterns. This fixes release failures where ARM task compiles the Bicep file but then reports `Could not find any file matching the template file pattern`. |
| 2026-04-04 | copilot | Fix infra release pipeline deployment scope mismatch: generated `main.bicep` has `targetScope = 'subscription'`, so shared ARM deploy template now uses `deploymentScope: 'Subscription'` and no longer requires or passes `resourceGroupName`. This fixes release failures like `Resource Group name should be provided` during `Validate - <config>-infra-*`. |
