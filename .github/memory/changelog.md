...
- [2026-05-13] `dev` — Review-gate remediation on `refacto/audit-12-05`: fixed multi-ACR existing-resource selection by carrying `TargetResourceId` through `ExistingResourceReference`, `GenerationRequestBuilder`, and project-level inference before matching `acrLoginServer` fallbacks in `ParentReferenceResolutionStage`; added existing-resource API-version coverage for Container Registry and Event Hub Namespace; narrowed `PipelineGenerationEngine` exception remapping to known validation cases only; aligned `ProjectPipelineAggregatorTests` with the new throw contract; reran focused tests plus full solution build/test green; and rechecked the touched slice with `review-expert` (no remaining findings).
- [2026-05-13] `dev` — GitHub closeout on `refacto/audit-12-05`: manually closed the final 21 `FIXED-IN-PR` audit issues (`#163`, `#166`, `#169`, `#175`, `#176`, `#182`, `#187`, `#188`, `#192`, `#197`, `#208`, `#209`, `#210`, `#211`, `#212`, `#215`, `#218`, `#219`, `#220`, `#222`, `#225`) with `completed`, resynced `audits/triage-2026-05-12.md` to `0` GitHub issues open / `0` real backlog, and confirmed the repository issue list is now empty.
- [2026-05-13] `dev` — Final audit-backlog wave on `refacto/audit-12-05`: closed `#189`, `#193`, `#202`, `#203`, `#217`, `#223`, `#227`, and `#295`; added local guards to `Name` / `EntraId`, made the three public EF navigations nullable (`AzureResource.ResourceGroup`, `ProjectEnvironmentDefinition.Project`, `ProjectMember.Project`) with the matching local repository dereference fix, hardened project-level `resourceType` validators against `AzureResourceTypes.All`, and resynced triage to 21 open GitHub issues / 0 real backlog items with full solution build/tests green.
- [2026-05-13] `dev` — DDD-007 / DDD-008 / APP-008 / APP-009 closed on `refacto/audit-12-05`: minimal in-process domain events (`AggregateRoot` + `ProjectCreatedDomainEvent` + `ProjectDbContext` dispatch), shared `AzureResource.SetNameAndLocation(...)` deduplication, authorization semantics closed by proof tests (read masked, write forbidden preserved), additive token-aware `IRepository.GetAllAsync(...)` overload, GitHub issues `#194`, `#195`, `#199`, `#200` closed, and triage dropped to 29 open issues / 8 real backlog items with full solution build/tests green.
...
- [2026-05-13] `dev` — Frontend build budget fix: updated `src/Front/angular.json` production budgets to `initial` `1.2 MB` / `1.3 MB` and `anyComponentStyle` `56 kB` / `64 kB`; `npm run build` now exits `0`, with only non-blocking OpenTelemetry CommonJS warnings remaining.
