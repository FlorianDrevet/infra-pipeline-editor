...
Stack Technique dotnet 10, ef core, scalar, Aspire, Angular 19 (frontend)
J'ai une api qui sauvegarde la config en base de donéne postgres sql, une api qui genere le bicep, et un frontend Angular dans `src/Front` qui consomme les endpoints.
...
Le frontend est structure par couches `core` (layouts) et `shared` (services/guards/facades/interfaces) dans `src/Front/src/app`.
Les URL backend sont centralisees dans `src/Front/src/environments/environment*.ts` et utilisees via `AxiosService`.
Si un contrat backend evolue, la consommation frontend correspondante doit etre mise a jour dans la meme modification.
