...
**Utiliser Graphify en premier** pour la compréhension structurelle et transversale, puis `read`/`search` pour le détail :

1. **Charger le skill `graphify-corpus`** (`.github/skills/graphify-corpus/SKILL.md`) si non déjà en contexte
2. Lire `graphify-out/GRAPH_REPORT.md` s'il existe
3. **`python -m graphify query "concept lié à la demande" --graph .\graphify-out\graph.json`** → identifier les flux et fichiers concernés
4. **`python -m graphify explain "SymboleCible" --graph .\graphify-out\graph.json`** → contextualiser les nœuds centraux
5. **`python -m graphify path "Source" "Cible" --graph .\graphify-out\graph.json`** → suivre les relations entre couches
6. Compléter avec `read`/`search` et les validations du projet pour le contenu exact et l'impact réel
...
| **Impact** | Utiliser Graphify pour repérer les relations et les fichiers concernés, puis confirmer l'impact avec `git diff`, le build et les tests. Quelles couches sont impactées ? Y a-t-il un risque de régression ? |
