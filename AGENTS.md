...
This repository uses Graphify as its single knowledge graph for source code, documentation, audits, diagrams, and cross-file relationships.

## Always Do

- Read `graphify-out/GRAPH_REPORT.md` before exploring source files when the report exists.
- Use Graphify queries to orient unfamiliar work: `python -m graphify query "concept" --graph .\graphify-out\graph.json`.
- Use `path` and `explain` for cross-file relationships and architectural context.
- Before modifying a shared symbol, query its callers and related concepts, then confirm the scope with focused tests, builds, and `git diff`.
- After changing source or documentation, refresh the graph with `python -m graphify update .`.

## When Debugging

1. Query the symptom with `python -m graphify query "<error or symptom>" --graph .\graphify-out\graph.json`.
2. Explain the suspected symbol with `python -m graphify explain "<symbol>" --graph .\graphify-out\graph.json`.
3. Trace relationships with `python -m graphify path "<source>" "<target>" --graph .\graphify-out\graph.json`.
4. Confirm the diagnosis with the smallest relevant test, build, or runtime check.

## Before Finishing

Verify that the graph was refreshed, the diff contains only expected files, and focused executable validation passed.

## Graphify Commands

| Task | Command |
|------|---------|
| Build or refresh the code graph | `python -m graphify update .` |
| Search a concept | `python -m graphify query "<question>" --graph .\graphify-out\graph.json` |
| Explain a node | `python -m graphify explain "<node>" --graph .\graphify-out\graph.json` |
| Find a path | `python -m graphify path "<source>" "<target>" --graph .\graphify-out\graph.json` |
| Serve the graph over MCP | `python -m graphify.serve .\graphify-out\graph.json` |
